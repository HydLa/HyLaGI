...
void MathematicaSolver::set_conditions(const node_sptr& constraint)
{
  ...
  HYDLA_LOGGER_VCS("%% setConditions");
  ml_.put_function("setConditions", 2);
  PacketSender ps(ml_);
...

  {
    ...
    for(; it!=variable_map.end(); ++it)
    {
      if(!it->second.undefined()){
        ...
        ps.put_value(it->second.get_unique(), VA_Prev);
      }
    }
    ps.put_vars();
    ml_.receive(); 
...
CheckConsistencyResult MathematicaSolver::check_consistency()
{
  ...
  case hydla::simulator::symbolic::DiscreteMode:
    ...
    break;
  case hydla::simulator::symbolic::ConditionsMode:
    ml_.put_function("checkConditions", 0);
    break;
  default:
...
node_sptr MathematicaSolver::receive_condition_node(ConditionsResult& node_type){
  ...
  switch(ml_.get_type()){ // 現行オブジェクトの型を得る
    ...
    HYDLA_LOGGER_REST("%% MLTKSYM(receive_condition_node)");
      std::string symbol = ml_.get_symbol();
      if(symbol=="False"){
        node_type = CONDITIONS_FALSE;
        return node_sptr();
      }else if(symbol=="True"){
        node_type = CONDITIONS_TRUE;
        return node_sptr();
      }else if(symbol=="t")
  ...
  if(ret != NULL){
    node_type = CONDITIONS_VARIABLE_CONDITIONS;
  }
  return ret;
}
...
void MathematicaSolver::receive_parameter_map(parameter_map_t &map){
  ...
  for(int cond_it = 0; cond_it < condition_size; cond_it++){
    ...
    range_t tmp_range = map[tmp_param];
