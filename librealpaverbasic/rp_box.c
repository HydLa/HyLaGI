...
/* Equality test between box x and box y 
 * Don't use                             */
int rp_box_vector_cmp(void *x, const void *y)
{
	int size_x, i;
	size_x = rp_box_size((rp_box)x);
	if(size_x == rp_box_size((rp_box)y))
	{
		for(i=0; i<size_x; i++)
		{
			if(!rp_interval_equal(rp_box_elem((rp_box)x,i), rp_box_elem((rp_box)y,i))) return 0;
		}
		return 1;
	}
	else
...
/* Destruction function for boxes in vectors */
void rp_box_vector_free(void * x)
{
	rp_box b = (rp_box)x;
	rp_box_destroy(&b);
}

/* Display function for boxes in vectors */
void rp_box_vector_display(FILE * out, void * x)
{
  rp_box b = (rp_box)x;
  rp_box_display_bounds(out, b, 8);
  fprintf(out, "\n");
}

void rp_vector_box_create(rp_vector * v)
{
	rp_vector_create(v,
					 rp_box_vector_cmp,
					 rp_box_vector_free,
					 rp_box_vector_display);
}
