...
/* Creation of a variable v equivalent to src -ohtani- */
void rp_variable_clone(rp_variable * v, rp_variable src)
{
	rp_variable_create(v, rp_variable_name(src));
	rp_variable_status(*v) = rp_variable_status(src);
	rp_variable_precision(*v) = rp_variable_precision(src);
	rp_variable_constrained(*v) = rp_variable_constrained(src);
	rp_union_copy(rp_variable_domain(*v), rp_variable_domain(src));
}
...
int rp_vector_variable_index(rp_vector v, const char *name)
{
	int i;
	for(i=0;i<rp_vector_size(v);i++)
	{
		if(rp_vector_cmp(v)(rp_vector_elem(v,i), name)) return i;
	}
	return -1;
}
