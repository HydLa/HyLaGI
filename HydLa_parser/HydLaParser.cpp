...
{
  file_iter_t fileBegin(filename);
  if (!fileBegin)	{    
    //    throw std::exception(filename);
  }
  file_iter_t fileEnd = fileBegin.make_end();

  pos_iter_t positBegin(fileBegin, fileEnd);
  pos_iter_t positEnd;

  HydLaGrammar hg;
  CommentGrammar cg;
  ast_tree_ = ast_parse<node_val_data_factory_t>(
    positBegin, positEnd, hg, cg);
...
bool HydLaParser::parse_string(const char* str)
{
  ...
  HydLaGrammar hg;
  ast_tree_ = ast_parse(str.c_str(), hg);
...
std::string HydLaParser::create_interlanguage(std::string max_time, bool debug)
{
  std::string str;
  str += "HydLaSolve[";
