...
REDUCEVCSPoint::REDUCEVCSPoint(REDUCEClient* cl) :
  cl_(cl)
{
}
...
/*
  if(Logger::mathsendflag==1){
	  HYDLA_LOGGER_AREA("---- Begin REDUCEVCSPoint::add_left_continuity_constraint ----");
  }

  HYDLA_LOGGER_DEBUG("---- Begin REDUCEVCSPoint::add_left_continuity_constraint ----");
...
{
	std::cout << "VCS上陸!!!" << std::endl;
/*
	if(Logger::constflag==9){
		HYDLA_LOGGER_AREA(
    "#*** Begin REDUCEVCSPoint::add_constraint ***");
	}
...
/*
  if(Logger::enflag==1||Logger::enflag==0){
     HYDLA_LOGGER_AREA(	"#*** REDUCEVCSPoint::check_entailment ***\n",
	"ask: ");
	 (negative_ask)->dump(std::cout);
	 HYDLA_LOGGER_AREA("\n");
	}
  HYDLA_LOGGER_DEBUG(
    "#*** REDUCEVCSPoint::check_entailment ***\n",
    "ask: ", *negative_ask);
