...
void MathematicaVCSInterval::send_init_cons(PacketSender &ps, const continuity_map_t &continuity_map){
  ...
  for(; md_it != md_end; ++md_it) {
    ...
    for(int i=0; i < (abs(md_it->second) + (md_it->second < 0)); i++)
    {
      ...
      constraints.push_back(MathematicaExpressionConverter::make_equal(variable_t(md_it->first, i), rhs, false));
    }
  }
  if(t_continuity_constraints.size() > 0){
    ml_->put_function("And", 2);
    ps.put_nodes(t_continuity_constraints, PacketSender::VA_Time);
  }
...
VCSResult MathematicaVCSInterval::check_consistency(const constraints_t& constraints)
{
  ...
  ml_->put_function("checkConsistencyTemporaryInterval", 2);
...
////////////////// 送信処理
  ...
  // 変数のリストを渡す
  ps.put_vars(PacketSender::VA_Zero);
