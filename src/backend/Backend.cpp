...
Backend::Backend(Link *link) { link_.reset(link); }
...
int Backend::read_args_fmt(const char *args_fmt, const int &idx, void *arg) {
  ...
  case 'p': {
    ...
    link_->put_parameter(par_prefix + par->get_name(),
                         par->get_differential_count(), par->get_phase_id());
  } break;
...
int Backend::call(const char *name, bool trace, int arg_cnt,
                  const char *args_fmt, const char *ret_fmt, ...) {
...
int Backend::send_node(const symbolic_expression::node_sptr &node,
...
int Backend::send_variable_map(const variable_map_t &vm, const VariableForm &vf,
  ...
  for (variable_map_t::const_iterator it = vm.begin(); it != vm.end(); it++) {
    ...
    if (!send_derivative && var.get_differential_count() > 0)
      continue;
    if (range.unique()) {
      size_to_sent++;
    } else {
...
int Backend::send_parameter_map(const parameter_map_t &parameter_map) {
  ...
  for (; it != parameter_map.end(); ++it) {
    if (it->second.unique()) {
      ...
      link_->put_parameter(par_prefix + param.get_name(),
...
void Backend::visit(boost::shared_ptr<Exists> node) {
  throw HYDLA_ERROR("exists node cannot be sent to backend");
}

void Backend::visit(boost::shared_ptr<Tell> node) { accept(node->get_child()); }
...
void Backend::visit(boost::shared_ptr<Function> node) {
  ...
  // if (!converted)
  //{
  //  throw HYDLA_ERROR(get_infix_string(node) + " is not suppported in " +
  //  link_->backend_name());
  //}
...
void Backend::visit(boost::shared_ptr<symbolic_expression::Parameter> node) {
  link_->put_parameter(par_prefix + node->get_name(),
...
void Backend::send_variable(const variable_t &var,
                            const VariableForm &variable_arg) {
  send_variable(var.get_name(), var.get_differential_count(), variable_arg);
}

void Backend::send_variable(const std::string &name, int diff_count,
                            const VariableForm &variable_arg) {
...
// コマンド文
void Backend::visit(boost::shared_ptr<symbolic_expression::PrintPP> node) {
  link_->put_symbol("True");
}
void Backend::visit(boost::shared_ptr<symbolic_expression::PrintIP> node) {
  link_->put_symbol("True");
}
void Backend::visit(boost::shared_ptr<symbolic_expression::Scan> node) {
  link_->put_symbol("True");
}

void Backend::visit(boost::shared_ptr<symbolic_expression::True> node) {
  link_->put_symbol("True");
}
void Backend::visit(boost::shared_ptr<symbolic_expression::False> node) {
  link_->put_symbol("False");
}

void Backend::set_range(const value_t &val, value_range_t &range,
                        const int &relop) {
  switch (relop) {
  case 0: // Equal
...
Backend::VariableForm Backend::adapt_variable_form(VariableForm form,
                                                   bool in_prev) {
...
void Backend::visit(
  ...
  boost::shared_ptr<symbolic_expression::ExpressionList> el =
      boost::dynamic_pointer_cast<symbolic_expression::ExpressionList>(
          node->get_lhs());
...
std::string Backend::remove_prefix(const std::string &original,
  ...
  if (original.length() <= prefix.length())
    throw HYDLA_ERROR("invalid name: " + original);
...
symbolic_expression::node_sptr Backend::receive_function() {
  ...
  {
    ret = symbolic_expression::node_sptr(
        new Divide(symbolic_expression::node_sptr(new Number("1")),
                   symbolic_expression::node_sptr(new Number("2"))));
    ret = symbolic_expression::node_sptr(
        new symbolic_expression::Power(receive_node(), ret));
    ...
    ret = symbolic_expression::node_sptr(
        new symbolic_expression::Parameter(name, differential_count, id));
  } else if (equal_ignoring_case(symbol, "prev")) {
    std::string name;
    name = remove_prefix(link_->get_symbol(), par_prefix);
    std::string d_str = link_->get_string();
    int differential_count = boost::lexical_cast<int, std::string>(d_str);
    symbolic_expression::node_sptr tmp_var =
        symbolic_expression::node_sptr(new symbolic_expression::Variable(name));
    for (int i = 0; i < differential_count; i++)
      tmp_var = symbolic_expression::node_sptr(
          new symbolic_expression::Differential(tmp_var));
    ret = symbolic_expression::node_sptr(
        new symbolic_expression::Previous(tmp_var));
  } else if (equal_ignoring_case(symbol, "minus")) {
    ret = symbolic_expression::node_sptr(
        new symbolic_expression::Negative(receive_node()));
             ...
             equal_ignoring_case(
                 symbol, "Complex")) { // 加減乗除など，二項演算子で書かれる関数
    ...
    for (int arg_it = 1; arg_it < arg_count; arg_it++) {
      ...
      if (equal_ignoring_case(symbol, "Plus"))
        ret = symbolic_expression::node_sptr(
            new symbolic_expression::Plus(lhs, rhs));
      else if (equal_ignoring_case(symbol, "Subtract"))
        ret = symbolic_expression::node_sptr(
            new symbolic_expression::Subtract(lhs, rhs));
      else if (equal_ignoring_case(symbol, "Times"))
        ret = symbolic_expression::node_sptr(
            new symbolic_expression::Times(lhs, rhs));
      else if (equal_ignoring_case(symbol, "Divide"))
        ret = symbolic_expression::node_sptr(
            new symbolic_expression::Divide(lhs, rhs));
      else if (equal_ignoring_case(symbol, "Power"))
        ret = symbolic_expression::node_sptr(
            new symbolic_expression::Power(lhs, rhs));
      else if (equal_ignoring_case(symbol, "Rational"))
        ret = symbolic_expression::node_sptr(
            new symbolic_expression::Divide(lhs, rhs));
      else if (equal_ignoring_case(symbol, "And"))
        ret = symbolic_expression::node_sptr(
            new symbolic_expression::LogicalAnd(lhs, rhs));
      else if (equal_ignoring_case(symbol, "Or"))
        ret = symbolic_expression::node_sptr(
            new symbolic_expression::LogicalOr(lhs, rhs));
      else if (equal_ignoring_case(symbol, "Equal"))
        ret = symbolic_expression::node_sptr(
            new symbolic_expression::Equal(lhs, rhs));
      else if (equal_ignoring_case(symbol, "Unequal"))
        ret = symbolic_expression::node_sptr(
            new symbolic_expression::UnEqual(lhs, rhs));
      else if (equal_ignoring_case(symbol, "Less"))
        ret = symbolic_expression::node_sptr(
            new symbolic_expression::Less(lhs, rhs));
      else if (equal_ignoring_case(symbol, "LessEqual"))
        ret = symbolic_expression::node_sptr(
            new symbolic_expression::LessEqual(lhs, rhs));
      else if (equal_ignoring_case(symbol, "Greater"))
        ret = symbolic_expression::node_sptr(
            new symbolic_expression::Greater(lhs, rhs));
      else if (equal_ignoring_case(symbol, "GreaterEqual"))
        ret = symbolic_expression::node_sptr(
            new symbolic_expression::GreaterEqual(lhs, rhs));
      else if (equal_ignoring_case(symbol, "Complex"))
        ret = symbolic_expression::node_sptr(new symbolic_expression::Plus(
            lhs, new symbolic_expression::Times(
                     rhs, new symbolic_expression::ImaginaryUnit())));
    }
  } else if (equal_ignoring_case(symbol, "derivative")) {
    std::string d_str = link_->get_string();
    int variable_differential_count =
        boost::lexical_cast<int, std::string>(d_str.c_str());
    std::string variable_name = link_->get_symbol();
    ret = symbolic_expression::node_sptr(
        new symbolic_expression::Variable(variable_name));
    for (int i = 0; i < variable_differential_count; i++) {
      ret = symbolic_expression::node_sptr(
          new symbolic_expression::Differential(ret));
    }
  } else {
    // その他の関数
...
symbolic_expression::node_sptr Backend::receive_node() {
...

  {
    ...
    if (symbol == "undefined") {
      ...
      ret =
          symbolic_expression::node_sptr(new symbolic_expression::SymbolicT());
    else if (symbol == "Pi")
      ...
      ret = symbolic_expression::node_sptr(
          new symbolic_expression::ImaginaryUnit());
    else if (symbol == "inf")
    ...
    else if (symbol.length() > var_prefix.length() &&
             symbol.substr(0, var_prefix.length()) == var_prefix)
      ret = symbolic_expression::node_sptr(new symbolic_expression::Variable(
          symbol.substr(var_prefix.length())));
    break;
  }
...

  {
    ...
    sstr << num;
    ret = symbolic_expression::node_sptr(
        new symbolic_expression::Number(sstr.str()));
    break;
  }
