...
public:
  ...
  enum DataType { DT_FUNC, DT_STR, DT_INT, DT_SYM, DT_NONE };
  ...
  void put_function(const std::string &name, int arg_cnt) {
    put_function(name.c_str(), arg_cnt);
  }
  virtual void put_parameter(const std::string &name, int diff_count,
                             int id) = 0;
  ...
  // std::string convert_function(const std::string& orig, bool hydla2back, bool
  // &converted)
  //{
  //  converted = true;
  //  return orig;
