...
bool UnaryNode::is_same_struct(const Node &n, bool exactly_same) const {
  return typeid(*this) == typeid(n) &&
         child_->is_same_struct(
             *static_cast<const UnaryNode *>(&n)->child_.get(), exactly_same);
}
...
struct BinaryNode::CheckInclude {
  ...
  template <typename T> bool operator()(T &it) {
...
bool BinaryNode::is_same_struct(const Node &n, bool exactly_same) const {
  ...
  if (typeid(*this) == typeid(n)) {
    ...
    for (; it != end; ++it) {
      if (std::find_if(target_node.begin(), target_node.end(),
...
bool Exists::is_same_struct(const Node &n, bool exactly_same) const {
  return is_exactly_same(n, exactly_same);
}
...
void VariadicNode::add_argument(node_sptr node) { arguments_.push_back(node); }

void VariadicNode::set_argument(node_sptr node, int i) { arguments_[i] = node; }
...
int VariadicNode::get_arguments_size() { return arguments_.size(); }
...
node_sptr Definition::clone() {
  ...
  copy(bound_variables_.begin(), bound_variables_.end(),
       n->bound_variables_.begin());
  n->child_ = child_->clone();
...
DEFINE_TREE_VISITOR_ACCEPT_FUNC(MulOfList);
