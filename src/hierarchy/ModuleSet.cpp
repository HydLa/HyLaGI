...
ModuleSet::ModuleSet(const std::string &name,
...
ModuleSet::module_list_const_iterator
ModuleSet::find(const module_t &mod) const {
  ...
  for (; it != end; ++it)
  {
    if (it->first == mod.first && it->second->get_id() == mod.second->get_id())
  return it;
  }
  return this->end();
  */
...
int ModuleSet::erase(const module_t &m) { return module_list_.erase(m); }
...
ModuleSet::module_list_const_iterator
ModuleSet::erase(const module_list_const_iterator &it) {
