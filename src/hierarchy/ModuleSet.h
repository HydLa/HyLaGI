...
class ModuleSet {
public:
  typedef std::pair<std::string, hydla::symbolic_expression::node_sptr>
      module_t;
  ...
  ModuleSet();
  ModuleSet(const std::string &name,
            hydla::symbolic_expression::node_sptr node);
   ...
   * 集合の最初の要素
   */
  module_list_const_iterator begin() const { return module_list_.begin(); }

  /**
   * 集合の最後の次の要素
   */
  module_list_const_iterator end() const { return module_list_.end(); }

  void clear() { module_list_.clear(); }
  ...
  void insert(const ModuleSet &ms) {
    module_list_.insert(ms.module_list_.begin(), ms.module_list_.end());
  }

  bool is_super_set(const ModuleSet &subset_mod) const {
    return std::includes(module_list_.begin(), module_list_.end(),
                         subset_mod.module_list_.begin(),
                         subset_mod.module_list_.end(), ModuleComp());
  }
  ...
  friend std::ostream &operator<<(std::ostream &s,
                                  const ModuleSet::module_t &m);

  size_t count(const module_t &m) const { return module_list_.count(m); }

  void insert(const module_t &m) { module_list_.insert(m); }
...
public:
  bool operator()(const ModuleSet &lhs, const ModuleSet &rhs) const {
