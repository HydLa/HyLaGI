 ...
 */
std::set<ModuleSet>
IncrementalModuleSet::get_removable_module_sets(ModuleSet &current_ms,
                                                const ModuleSet &ms) {
...
void IncrementalModuleSet::add_parallel(
    IncrementalModuleSet &parallel_module_set) {
...
void IncrementalModuleSet::add_weak(
    IncrementalModuleSet &weak_module_set_list) {
...
std::ostream &
IncrementalModuleSet::dump_module_sets_for_graphviz(std::ostream &s) {
  reset();
...
std::ostream &
IncrementalModuleSet::dump_priority_data_for_graphviz(std::ostream &s) const {
  s << "digraph priority_data { " << std::endl;
  s << "  edge [dir=back];" << std::endl;
  for (auto m : weaker_modules_) {
    for (auto wm : m.second) {
      s << "  \"" << m.first.first << "\" -> \"" << wm.first << "\";"
        << std::endl;
    }
  }
  s << "}" << std::endl;
...
std::ostream &IncrementalModuleSet::dump(std::ostream &s) const { return s; }
...
bool IncrementalModuleSet::check_same_ms_generated(module_set_set_t &new_mss,
                                                   ModuleSet &ms) {
 ...
 * @param ms 矛盾の原因となったモジュール集合
 */
void IncrementalModuleSet::generate_new_ms(const module_set_set_t &mcss,
                                           const ModuleSet &ms) {
  ...
  for (auto lit : ms_to_visit_) {
    ...
    if (lit.disjoint(inconsistent_ms)) {
      ...
      for (auto removable_module_set : rm) {
        ...
        // check weather the new_ms is included by maximal consistent module
        // sets.
        bool checked = false;
        for (auto mcs : mcss) {
