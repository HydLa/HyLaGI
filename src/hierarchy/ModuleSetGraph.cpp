...
void ModuleSetGraph::add_required_parallel(
    ModuleSetGraph &parallel_module_set_graph) {
...
namespace {
struct ModSizePred {
  ModSizePred(size_t size) : size_(size) {}
...
void ModuleSetGraph::build_edges() {
  ...
  if (full_module_set_set_.size() == 0)
    return;
  ...
  while (super_it != node_end) {
    ...
    for (; super_it != super_end; ++super_it) {
      for (module_set_set_t::reverse_iterator tmp_subset_it = subset_it;
           tmp_subset_it != subset_end; ++tmp_subset_it) {
        if ((*super_it).including(*tmp_subset_it)) {
          edges_.insert(edges_t::value_type((*super_it), (*tmp_subset_it)));
        }
      }
    }
...
std::ostream &ModuleSetGraph::dump_edges(std::ostream &s) const {
  ...
  s << "{";
  for (; it != end; ++it) {
    s << it->first.get_name() << "->" << it->second.get_name() << "\n";
  }
  s << "}";
