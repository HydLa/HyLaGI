...
Json_p::json_p(std::stringstream &sout, std::string prio) {
  value jstr;
...
Json_p::make_prio_map(std::string prio) {
  ...
  while (1) { /// 直接階層が設定されていない制約も確認する
    ...
    for (auto p : ret) {
      for (auto p1 : p.second) {
        for (auto p2 : ret[p1]) {
          if (std::find(p.second.begin(), p.second.end(), p2) ==
                  p.second.end() and
              p2 != p.first) {
...
std::vector<std::vector<std::string>>
Json_p::make_h_map(std::vector<std::vector<std::vector<std::string>>> v_ret,
                   std::map<std::string, std::vector<std::string>> p_ret) {
  std::vector<std::vector<std::string>> ret, h_ret;
  std::vector<std::string> each_cons, h;
  std::string
      ask_flag; // []がつかない制約は0, それ以外でask制約があれば1, なければ2
...
Json_p::make_vector(std::string serch_str) {
  ...
  std::vector<std::string> split_str, split_str2, split_str21, split_str3,
      split_str3_ask, split_str3_tell, cons_list;
  std::string cons;
  ...
  for (auto spl : split_str) {
    ...
    if (std::find(cons_list.begin(), cons_list.end(), cons) !=
        cons_list.end()) { // 登録済みなら飛ばす
      continue;
    }
    each_eq.push_back(cons);
    cons_list.push_back(cons);
    split(
        split_str21, split_str2[1],
        logical_and_tell); /// 複数tell制約がある場合の対処(複数のask制約には対応していない)
...
std::string Json_p::json_search(value jstr, bool always, bool ask) {
  ...
  std::vector<std::string> ineq = {"Equal", "Less", "LessEqual", "Greater",
                                   "GreaterEqual"};
...
std::string Json_p::make_equation(value jstr) {
  std::vector<std::string> lrhs = {
      "Subtract", "Plus",      "Divide",  "Times",        "Equal",
      "Less",     "LessEqual", "Greater", "GreaterEqual", "Power"};
...
std::vector<std::vector<std::string>>
Json_p::make_v_map(std::shared_ptr<hydla::parse_tree::ParseTree> parse_tree) {
