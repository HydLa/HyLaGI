...
public:
   ...
   */
  void update(parse_tree::ParseTree* pt);
  ...
  /// 任意数の子ノードを持つノードの呼び出し
  virtual void visit(boost::shared_ptr<symbolic_expression::ArbitraryNode> node);
...
private:  
  ...
  {
    symbolic_expression::node_id_t id = n->get_id();
    if(id == 0) {
