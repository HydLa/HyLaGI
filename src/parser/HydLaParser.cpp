...
Parser::Parser(std::string file, std::vector<std::string> str)
    : lexer(file, str) {}
Parser::~Parser() {}
...
node_sptr Parser::parse(node_sptr &an,
  ...
  if (error_info.empty()) {
    ...
    for (auto d : program_definitions)
      tmp_program_definitions.push_back(d);
    for (auto d : constraint_definitions)
      tmp_constraint_definitions.push_back(d);
    for (auto d : program_list_definitions)
      tmp_program_list_definitions.push_back(d);
    for (auto d : expression_list_definitions)
      tmp_expression_list_definitions.push_back(d);
...
node_sptr Parser::hydla_program() { return statements(); }
...
node_sptr Parser::statements() {
  ...
  if (!parse_ended()) {
    ...
    while (!parse_ended()) {
      position_t position = lexer.get_current_position();
      if ((constraint_callee()) || (program_callee()) ||
          (program_list_callee()) || (expression_list_callee())) {
        Token token = lexer.get_token();
        if (token == LEFT_BRACES || token == EQUIVALENT ||
            token == DEFINITION) {
          lexer.set_current_position(position);
          break;
        }
...
node_sptr Parser::statement() {
  ...
  if ((cd = constraint_callee())) {
    ...
    if (lexer.get_token() == EQUIVALENT) {
      if ((is_defined(cd)) && !second_parse) {
        error_occurred(tmp_position,
                       "redefinition of " +
                           std::to_string(cd->bound_variable_size()) +
                           " args definition \"" + cd->get_name() + "\"");
      } else {
        ...
        if ((tmp = constraint())) {
          ...
          error_occurred(tmp_position,
                         "expected \".\" after constraint definition");
          return node_sptr();
        }
    ...
    if (lexer.get_token() == LEFT_BRACES) {
      if ((is_defined(pd)) && !second_parse) {
        error_occurred(lexer.get_current_position(),
                       "redefinition of " +
                           std::to_string(pd->bound_variable_size()) +
                           " args definition \"" + pd->get_name() + "\"");
      } else {
        tmp_position = lexer.get_current_position();
        node_sptr tmp;
        // program
  ...
  if ((pld = program_list_callee())) {
    ...
    if (lexer.get_token() == DEFINITION) {
      if ((is_defined(pld)) && !second_parse) {
        error_occurred(tmp_position,
                       "redefinition of " +
                           std::to_string(pld->bound_variable_size()) +
                           " args definition \"" + pld->get_name() + "\"");
      } else {
        node_sptr tmp;
        tmp_position = lexer.get_current_position();
        if ((tmp = program_list())) {
          ...
          error_occurred(tmp_position,
                         "expected \".\" after program list definition");
        } else {
  ...
  if ((eld = expression_list_callee())) {
    ...
    if (lexer.get_token() == DEFINITION) {
      if ((is_defined(eld)) && !second_parse) {
        error_occurred(tmp_position,
                       "redefinition of " +
                           std::to_string(eld->bound_variable_size()) +
                           " args definition \"" + eld->get_name() + "\"");
      } else {
        node_sptr tmp;
        tmp_position = lexer.get_current_position();
        if ((tmp = expression_list())) {
          ...
          error_occurred(tmp_position,
                         "expected \".\" after expression list definition");
        } else {
          error_occurred(tmp_position, "expected list after \":=\"");
        }
      }
  ...
  if ((ret = program())) {
    ...
    if (lexer.get_token() == PERIOD) {
      if (!parsed_program)
        ...
        parsed_program =
            std::shared_ptr<Parallel>(new Parallel(parsed_program, ret));
      return ret;
    }
  ...
  error_occurred(
      ...
      "expected definition statement, assertion statement or program");
  return node_sptr();
}
 ...
 */
node_sptr Parser::program_factor() { return module(); }
    ...
    if ((ret = program())) {
      ...
      if (lexer.get_token() == RIGHT_PARENTHESES) {
        ...
        while ((token = lexer.get_token()) == RIGHT_PARENTHESES)
          ;
        if (token == COMMA || token == WEAKER || token == RIGHT_BRACES ||
            token == PERIOD || token == VERTICAL_BAR) {
...
node_sptr Parser::module() {
  ...
  if ((ret = program_caller())) {
    ...
    if (token == COMMA || token == WEAKER || token == RIGHT_BRACES ||
      ...
      error_occurred(
          tmp_position,
          "\",\", \"<<\", \"}\" and \".\" must be after program_caller");
    }
  }
  lexer.set_current_position(position);

  // "(" program ")"
  if ((ret = parenthesis_program())) {
    return ret;
  }

  // program_lists
  if ((ret = program_list_element())) {
    return ret;
  }
  if ((ret = program_list())) {
    return ret;
...
std::shared_ptr<ProgramCaller> Parser::program_caller() {
  ...
  if ((name = definition_name()) != "") {
    ...
    if (second_parse) {
      ...
      error_occurred(lexer.get_current_position(),
                     "undefined program - " + std::to_string(args.size()) +
                         " args program \"" + name + "\"");
    } else
      return ret;
  }
...
std::string Parser::definition_name() {
  ...
  if (lexer.get_token() == UPPER_IDENTIFIER)
    return lexer.get_current_token_string();
  lexer.set_current_position(position);
  return std::string();
}
