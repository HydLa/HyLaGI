...
public:
  ...
  variable_map_t get_variable_map() const { return variable_map_; }
  ...
  swap_tree(const symbolic_expression::node_sptr &tree);
  ...
  void dispatch(symbolic_expression::TreeVisitor *visitor) {
    if (node_tree_)
      node_tree_->accept(node_tree_, visitor);
  }
   ...
   * トップノードを得る
   */
  symbolic_expression::node_sptr get_node() const { return node_tree_; }
