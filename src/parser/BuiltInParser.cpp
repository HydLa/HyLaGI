...
node_sptr Parser::constant() {
  ...
  if (lexer.get_token() == UPPER_IDENTIFIER) {
    ...
    if (str == "Pi")
      return std::shared_ptr<Pi>(new Pi());
    if (str == "Infinity")
      return std::shared_ptr<Infinity>(new Infinity());
    if (str == "E")
...
node_sptr Parser::system_variable() {
  ...
  if (lexer.get_token() == SYSTEM) {
    if (lexer.get_token() == LOWER_IDENTIFIER) {
      ...
      // t
      if (str == "t") {
        return std::shared_ptr<SymbolicT>(new SymbolicT());
      }
      // timer
      if (str == "timer") {
        return std::shared_ptr<SVtimer>(new SVtimer());
      }
    }
  }
  lexer.set_current_position(position);
...
node_sptr Parser::tautology() {
  ...
  if (lexer.get_token() == SYSTEM) {
    if (lexer.get_token() == UPPER_IDENTIFIER) {
      // "TRUE"
      if (lexer.get_current_token_string() == "TRUE") {
        return std::shared_ptr<True>(new True());
      }
      if (lexer.get_current_token_string() == "FALSE") {
        return std::shared_ptr<False>(new False());
      }
    }
  }
  lexer.set_current_position(position);
