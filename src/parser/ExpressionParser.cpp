...
node_sptr Parser::compare_expression()
{
  ...
  if ((lhs = expression()))
  {
    ...
    // ("<"|"<="|">"|">="|"="|"!=")
    if (token == LESS || token == LESS_EQUAL || token == GREATER || token == GREATER_EQUAL || token == EQUAL || token == NOT_EQUAL)
    {
      node_sptr rhs;
      do
      {
        ...
        if ((rhs = expression()))
        {
          ...
          if (!ret)
            ...
            ret = boost::shared_ptr<LogicalAnd>(new LogicalAnd(ret, lhs));
          lhs = rhs;
        }
        else
        {
      ...
      } while (token == LESS || token == LESS_EQUAL || token == GREATER || token == GREATER_EQUAL || token == EQUAL || token == NOT_EQUAL);
...
node_sptr Parser::power()
{
  ...
  if ((ret = prev()))
  {
    ...
    if (lexer.get_token() == POWER)
    {
      ...
      if ((tmp = power()))
      {
        return boost::shared_ptr<Power>(new Power(ret, tmp));
      }
      else
      {
        error_occurred(lexer.get_current_position(), "expected expression after \"" + op_token + "\"");
...
node_sptr Parser::prev()
{
  ...
  if ((ret = diff()))
  {
    ...
    if (lexer.get_token() == MINUS)
    {
      ...
      if (!(factor()))
      {
        return boost::shared_ptr<Previous>(new Previous(ret));
      }
    }
    lexer.set_current_position(position);
    return ret;
...
boost::shared_ptr<Variable> Parser::variable()
{
  ...
  if (name != "")
  {
    ...
    if (lexer.get_token() != LEFT_BOX_BRACKETS)
    {
      lexer.set_current_position(tmp_position);
      if (name == "t")
        return boost::shared_ptr<Variable>(new SymbolicT());
...
node_sptr read_floating_point_num(const std::string& str)
{
  std::string num = "";
...
{
  return read_floating_point_num(std::to_string(num));
}
...
node_sptr Parser::number()
{
  ...
  if (token == NUMBER)
  {
    ...
    return boost::shared_ptr<Divide>(new Divide(boost::shared_ptr<Number>(new Number(num)), boost::shared_ptr<Number>(new Number(divided_by))));*/
    return read_floating_point_num(lexer.get_current_token_string());
  }
  if (token == INTEGER)
  {
