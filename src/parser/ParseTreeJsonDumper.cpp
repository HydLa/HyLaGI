...
// Exists
void ParseTreeJsonDumper::visit(boost::shared_ptr<Exists> node) {
  dump_node(node);
}
...
// 円周率
void ParseTreeJsonDumper::visit(boost::shared_ptr<Pi> node) { dump_node(node); }

// 自然対数の底
void ParseTreeJsonDumper::visit(boost::shared_ptr<E> node) { dump_node(node); }
