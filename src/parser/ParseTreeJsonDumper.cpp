...
void ParseTreeJsonDumper::dump_node(
...
// 円周率
void ParseTreeJsonDumper::visit(std::shared_ptr<symbolic_expression::Pi> node) {
  dump_node(node);
}

// 自然対数の底
void ParseTreeJsonDumper::visit(std::shared_ptr<symbolic_expression::E> node) {
  dump_node(node);
}
