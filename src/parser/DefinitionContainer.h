 ...
 * 定義制約を収納するクラス
 */
template <typename DefinitionNodeType> class DefinitionContainer {
public:
  typedef DefinitionNodeType definition_node_t;

  typedef std::string definition_name_t;
  typedef int bound_variable_count_t;
  typedef std::pair<definition_name_t, bound_variable_count_t>
      definition_map_key_t;

  typedef std::shared_ptr<definition_node_t> definition_map_value_t;
  typedef std::map<definition_map_key_t, definition_map_value_t>
      definition_map_t;
  typedef
      typename definition_map_t::const_iterator definition_map_const_iterator;
  ...
  definition_map_key_t create_definition_key(
      const std::shared_ptr<hydla::symbolic_expression::Definition> &d) const {
    return std::make_pair(d->get_name(), d->bound_variable_size());
  }
   ...
   */
  void clear() { definition_map_.clear(); }
  ...
  definition_map_const_iterator begin() const {
    return definition_map_.begin();
  }
