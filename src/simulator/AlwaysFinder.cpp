...
void AlwaysFinder::find_always(symbolic_expression::node_sptr node,
                               always_set_t *al, ConstraintStore *non_al,
                               asks_t *pos, asks_t *nonal_asks) {
  always_set = al;
  non_always = non_al;
  positive_asks = pos;
...
void AlwaysFinder::visit_atomic_constraint(
    std::shared_ptr<symbolic_expression::Node> node) {
  if (non_always != nullptr)
    non_always->add_constraint(node);
}
