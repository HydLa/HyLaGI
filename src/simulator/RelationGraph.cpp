        ...
        else
        {
          ...
          for (auto edge : tell_node->edges)
          {
            if (!ignore_prev || !edge.ref_prev)
              collect_node(edge.variable_node, &connected_constraints, &connected_ms, nullptr, false);
          }
          if (connected_constraints.size() > 0)
          {
            constraints_vector.push_back(connected_constraints);
...
void RelationGraph::collect_node(TellNode *node, ConstraintStore *constraints, module_set_t *module_set, variable_set_t *vars, bool visit_guards)
{
  ...
  {
    if (!ignore_prev || !edge.ref_prev)
      collect_node(edge.variable_node, constraints, module_set, vars, visit_guards);
  }
...
void RelationGraph::collect_node(VariableNode *node, ConstraintStore *constraints, module_set_t *module_set, variable_set_t *vars, bool visit_guards)
{
  ...
  for (auto edge : node->edges)
  {
    if ((!ignore_prev || !edge.ref_prev) && !edge.tell_node->collected && edge.tell_node->is_active())
...
{
  HYDLA_LOGGER_DEBUG("BREAK set_expanded_atomic - ", *cons);
...
void RelationGraph::set_entailed(const ask_t &ask, bool entailed)
{
  ...
  if (node_it != ask_node_map.end())
  {
    ...
    throw HYDLA_ERROR("AtomicGuardNode for " + get_infix_string(ask) + " is not found");
}
...
void RelationGraph::remove(const constraint_t &eraseGuard)
{
  const bool debugLog = true;

  HYDLA_LOGGER_DEBUG("BREAK Erase guard - ", get_infix_string(eraseGuard));

  const auto resetEdges = [&](std::vector<EdgeToConstraint> &edges, const std::string &str) {
    for (auto it = edges.begin(); it != edges.end();)
    {
      TellNode *pTellNode = it->tell_node;
      const bool result = pTellNode->tell->has_sub_tree(*eraseGuard);
      if (debugLog /*  && result && result result*/)
        HYDLA_LOGGER_DEBUG(str, "var.edges - ", get_infix_string(pTellNode->tell), " : ", result ? "true" : "false");
      if (result)
      {
        tell_nodes.erase(std::remove_if(tell_nodes.begin(), tell_nodes.end(), [&](TellNode *p) {
                           return pTellNode->tell->is_same_struct(*(p->tell), false);
                         }),
                         tell_nodes.end());
        it = edges.erase(it);
      }
  ...
  const auto resetAsks = [&](std::vector<AskNode *> &ask_edges, const std::string &str) {
    for (auto it = ask_edges.begin(); it != ask_edges.end();)
    {
      bool hasEraseGuard = false;
      for (AtomicGuardNode *p : (*it)->atomic_guard_list)
      {
        const bool result = p->atomic_guard.constraint->has_sub_tree(*eraseGuard);
        if (debugLog /*  && result && result result*/)
          HYDLA_LOGGER_DEBUG(str, "var.ask_edges - ", get_infix_string(p->atomic_guard.constraint), " : ", result ? "true" : "false");
        if (result)
        {
          hasEraseGuard = true;
          break;
      ...
      if (hasEraseGuard)
      {
        const auto tells = (*it)->children;
        for (ConstraintNode *pTellConstraint : tells)
        {
          TellNode *pTellNode = dynamic_cast<TellNode *>(pTellConstraint);
          if (pTellNode)
          {
            tell_nodes.erase(std::remove_if(tell_nodes.begin(), tell_nodes.end(), [&](TellNode *p) {
                               return pTellNode->tell->is_same_struct(*(p->tell), false);
  ...
  for (auto &keyval : var_name_nodes_map)
  {
    //keyval.first;
    //keyval.second;
    //std::list<VariableNode*>
    for (VariableNode *pNode : keyval.second)
    {
      resetEdges(pNode->edges, "var_name_nodes_map ");
      resetAsks(pNode->ask_edges, "var_name_nodes_map ");
      //var.edges;
      //EdgeToConstraint
      //  TellNode*
      //    constraint_t
  ...
  //module_ask_nodes_map
  for (auto it = module_ask_nodes_map.begin(); it != module_ask_nodes_map.end();)
  {
    bool hasEraseGuard = false;
    for (AskNode *pAskNode : it->second)
    {
      for (AtomicGuardNode *p : pAskNode->atomic_guard_list)
      {
        const bool result = p->atomic_guard.constraint->has_sub_tree(*eraseGuard);
        if (debugLog /*  && result && result result*/)
          HYDLA_LOGGER_DEBUG("module_ask_nodes - ", get_infix_string(p->atomic_guard.constraint), " : ", result ? "true" : "false");
        if (result)
        {
          hasEraseGuard = true;
          break;
    ...
    hasEraseGuard = hasEraseGuard || it->first.second->has_sub_tree(*eraseGuard);

    if (hasEraseGuard)
    {
      it = module_ask_nodes_map.erase(it);
    }
    else
    {
      ++it;
  ...
  //ask_node_map
  for (auto it = ask_node_map.begin(); it != ask_node_map.end();)
  {
    bool hasEraseGuard = false;
    for (AtomicGuardNode *p : (it->second)->atomic_guard_list)
    {
      const bool result = p->atomic_guard.constraint->has_sub_tree(*eraseGuard);
      if (debugLog /*  && result && result result*/)
        HYDLA_LOGGER_DEBUG("ask_nodes - ", get_infix_string(p->atomic_guard.constraint), " : ", result ? "true" : "false");
      if (result)
      {
        hasEraseGuard = true;
        break;
    ...
    hasEraseGuard = hasEraseGuard || it->first->has_sub_tree(*eraseGuard);

    if (hasEraseGuard)
    {
      it = ask_node_map.erase(it);
    }
    else
    {
      ++it;
  ...
  //atomic_guard_list
  for (auto it = atomic_guard_list.begin(); it != atomic_guard_list.end();)
  {
    const bool result = (*it)->atomic_guard.constraint->has_sub_tree(*eraseGuard);
    if (debugLog /*  && result && result result*/)
      HYDLA_LOGGER_DEBUG("ask_nodes - ", get_infix_string((*it)->atomic_guard.constraint), " : ", result ? "true" : "false");
    if (result)
    {
      it = atomic_guard_list.erase(it);
    }
    else
    {
      ++it;
  ...
  //tell_node_map
  for (auto it = tell_node_map.begin(); it != tell_node_map.end();)
  {
    const bool result = it->first->has_sub_tree(*eraseGuard);
    if (debugLog /*  && result && result result*/)
      HYDLA_LOGGER_DEBUG("tell_node_map - ", get_infix_string(it->first), " : ", result ? "true" : "false");
    if (result)
    {
      it = tell_node_map.erase(it);
    }
    else
    {
      ++it;
  ...
  //constraint_node_map
  for (auto it = constraint_node_map.begin(); it != constraint_node_map.end();)
  {
    const bool result = it->first->has_sub_tree(*eraseGuard);
    if (debugLog /*  && result && result result*/)
      HYDLA_LOGGER_DEBUG("constraint_node_map - ", get_infix_string(it->first), " : ", result ? "true" : "false");
    if (result)
    {
      it = constraint_node_map.erase(it);
    }
    else
    {
      ++it;
  ...
  //guard_node_map
  for (auto it = guard_node_map.begin(); it != guard_node_map.end();)
  {
    const bool result = it->first->has_sub_tree(*eraseGuard);
    if (debugLog /*  && result && result result*/)
      HYDLA_LOGGER_DEBUG("guard_node_map - ", get_infix_string(it->first), " : ", result ? "true" : "false");
    if (result)
    {
      it = guard_node_map.erase(it);
    }
    else
    {
      ++it;
  ...
  for (auto it = tell_nodes.begin(); it != tell_nodes.end();)
  {
    const bool result = (*it)->tell->has_sub_tree(*eraseGuard);
    if (debugLog /*  && result && result result*/)
      HYDLA_LOGGER_DEBUG("tell_nodes - ", get_infix_string((*it)->tell), " : ", result ? "true" : "false");
    if (result)
    {
      it = tell_nodes.erase(it);
    }
    else
    {
      ++it;
  ...
  for (auto it = ask_nodes.begin(); it != ask_nodes.end();)
  {
    bool hasEraseGuard = false;
    for (AtomicGuardNode *p : (*it)->atomic_guard_list)
    {
  ...
  for (auto it = guard_nodes.begin(); it != guard_nodes.end();)
  {
    auto constraints = (*it)->get_atomic_guards();
    bool hasEraseGuard = false;
    for (AtomicConstraint *p : constraints)
    {
      const bool result = p->constraint->has_sub_tree(*eraseGuard);
      if (debugLog /*  && result && result result*/)
        HYDLA_LOGGER_DEBUG("guard_nodes - ", get_infix_string(p->constraint), " : ", result ? "true" : "false");
      if (result)
      {
        hasEraseGuard = true;
        break;
  ...
  //  ConstraintStore
  //  [ConstraintNode*]
  for (auto pVar : variable_nodes)
  {
    auto &edges = pVar->edges;
    resetEdges(edges, "variable_nodes ");

    auto &ask_edges = pVar->ask_edges;
    resetAsks(ask_edges, "variable_nodes ");
  }
}
...
void RelationGraph::visit_atomic_constraint(boost::shared_ptr<symbolic_expression::Node> node)
{
  ...
  else if (visit_mode == EXPANDING)
  {
    ...
    if (tell_node_it != tell_node_map.end())
    {
      ...
      HYDLA_LOGGER_WARN("(@RelationGraph) try to expand unknown node: ", get_infix_string(node));
  }
  else if (visit_mode == UNEXPANDING)
  {
    ...
    if (tell_node_it != tell_node_map.end())
    {
      ...
      HYDLA_LOGGER_WARN("(@RelationGraph) try to unexpand unknown node: ", get_infix_string(node));
  }
}
