...
PrevReplacer::PrevReplacer(PhaseResult &phase, Simulator &simulator,
                           backend::Backend *b, bool affine)
    : prev_phase(phase), simulator(simulator), backend(b), affine_mode(affine) {
}
...
void PrevReplacer::visit(std::shared_ptr<symbolic_expression::Previous> node) {
  assert(!in_prev);
  in_prev = true;
...
void PrevReplacer::visit(std::shared_ptr<symbolic_expression::Variable> node) {
  if (in_prev) {
    ...
    if (range.unique()) {
      ...
      if (affine_mode) {
        ...
        if (range.get_upper_cnt() != 1 || range.get_lower_cnt() != 1) {
          HYDLA_LOGGER_ERROR("invalid range for affine arithmetic mode, " +
                             range.get_string());
        }
        hydla::backend::MidpointRadius mr;
        value_t lb = range.get_lower_bound(0).value;
        value_t ub = range.get_upper_bound(0).value;
        backend->call("intervalToMidpointRadius", false, 2, "vltvlt", "r", &lb,
                      &ub, &mr);
        ...
        new_child =
            symbolic_expression::node_sptr(new symbolic_expression::Parameter(
                v_name, diff_cnt, prev_phase.id));
      }
      ...
      if (!simulator.get_parameter_map().count(param)) {
        ...
        ConstraintStore par_cons = range.create_range_constraint(node_sptr(
            new symbolic_expression::Parameter(param.get_name(),
