...
AutomatonNode::AutomatonNode(phase_result_sptr_t phase, std::string name,
                             int id) {
  this->id = id;
  this->name = name;
  this->phase = phase;
...
void AutomatonNode::set_id(int id) { this->id = id; }

void AutomatonNode::set_name(std::string name) { this->name = name; }

void AutomatonNode::set_color(std::string color) { this->color = color; }

void AutomatonNode::set_peripheries(int num) { this->peripheries = num; }
    ...
    for (auto it = edges.begin(); it != edges.end(); it++) {
      ...
      if (this->color == (*it).first->color) {
        ost << "\"" << name << "\""
            ...
            << get_infix_string((*it).second) << "\"];" << endl;
      } else {
        ost << "\"" << name << "\""
            << " -> "
            << "\"" << nextname << "\""
            << "[label=\"" << get_infix_string((*it).second) << "\"];" << endl;
      }
    }
  } else {
    for (auto it = edges.begin(); it != edges.end(); it++) {
...
void AutomatonNode::remove() {
  for (auto node : reversed_edges) {
    for (auto edge_it = node->edges.begin(); edge_it != node->edges.end();
         edge_it++) {
...
void Automaton::clone_node(AutomatonNode *original_node,
                           map<int, AutomatonNode *> &cloned_nodes) {
  ...
  for (auto edge : original_node->edges) {
    ...
    cloned_node->edges.push_back(
        make_pair(cloned_nodes[node->id], edge.second));
  }
}
...
void Automaton::get_nodes(AutomatonNode *node,
                          list<AutomatonNode *> &result_list) {
  visited_nodes.insert(node);
  result_list.push_back(node);
  for (auto edge : node->edges) {
    if (!visited_nodes.count(edge.first))
      get_nodes(edge.first, result_list);
  }
}
...
void AutomatonNode::trace(){
  ...
  for(typename std::vector<AutomatonNode *>::iterator it =
this->trace_path.begin();it + 1 != this->trace_path.end();it++){ if(it ==
this->trace_path.begin()){ std::cout << "\"init\"[shape=\"point\"];" <<
std::endl; std::cout << "\"init\"" << " -> " << "\"" << (*it)->name << "\"" <<
"[color=\"" << (*it)->color << "\"];" << std::endl;
    }
    if((it + 1) != trace_path.end()){
      std::cout << "\"" << (*it)->name << "\"" << " " << "[color=\"" <<
(*it)->color << "\"];" << std::endl; if((*it)->color == (*(it + 1))->color){
        std::cout << "\"" << (*it)->name << "\"" << " -> " << "\"" << (*(it +
1))->name << "\"" << "[color=\"" << (*it)->color << "\"];" << std::endl; }else{
        std::cout << "\"" << (*it)->name << "\"" << " -> " << "\"" << (*(it +
1))->name << "\"" << ";" << std::endl;
      }
    }
...
void Automaton::dump(){
  std::cout << "~~~~~~~~ dump : " << this->name << " start. ~~~~~~~~" <<
std::endl; std::cout << "digraph g{" << std::endl; this->trace();
  this->write_reset();
  std::cout << "// nodes and links after " << this->name << "." << std::endl;
  this->dump_node_and_edge();
  this->write_reset();
  std::cout << "}" << std::endl;
  std::cout << "~~~~~~~~ dump : " << this->name << " end.  ~~~~~~~~" <<
std::endl;
}
...
void Automaton::output_dot(){
  ...
  std::cout << "\"init\"" << " -> " << "\"" << this->name << "\"" << "[color=\""
<< this->color << "\"];" << std::endl; this->dump_node_and_edge(); std::cout <<
"}" << std::endl; this->write_reset(); std::cout <<
"==================================" << std::endl;
}
...
void AutomatonNode::dump_node_and_edge(){
  if(write == 0){
    write++;
    std::cout << "\"" << this->name << "\"" << " " << "[color=\"" << this->color
<< "\"];" << std::endl; for(typename std::vector<std::pair<AutomatonNode
*,node_sptr>>::iterator it = next_edge.begin();it != next_edge.end();it++){
      if(this->name == (*it).first->name){
        std::cout << "\"" << this->name << "\"" << " -> " << "\"" << this->name
<< "\"" << " " <<  "[color=\"" << this->color << "\"];" << std::endl; }else{
        if(this->color == (*it).first->color){
          std::cout << "\"" << this->name << "\"" << " -> " << "\"" <<
(*it).first->name << "\"" << "[color=\"" << this->color << "\"];" << std::endl;
        }else{
          std::cout << "\"" << this->name << "\"" << " -> " << "\"" <<
