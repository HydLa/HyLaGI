...
struct NodeDumper {
  ...
  template <typename T> NodeDumper(T it, T end) {
    for (; it != end; ++it) {
      ss << hydla::symbolic_expression::get_infix_string(*it) << "\n";
    }
  }

  NodeDumper(const NodeDumper &rhs) { ss << rhs.ss.rdbuf(); }
...
PhaseResult::PhaseResult()
    : phase_type(InvalidPhase), simulation_state(NOT_SIMULATED),
      parent(nullptr) {}
...
void PhaseResult::generate_full_information() const {
  ...
  for (auto r_it = ancestor_list.rbegin(); r_it != ancestor_list.rend();
       r_it++) {
...
std::vector<parameter_map_t> PhaseResult::get_parameter_maps() const {
  if (!parameter_maps) {
    if (parameter_constraint.size() == 0) {
      ...
      backend->call("createParameterMaps", true, 1, "csn", "mps",
                    &parameter_constraint, &par_maps);
...
ostream &operator<<(std::ostream &os, const pp_time_result_t &result) {
  for (auto candidate : result) {
    ...
    for (auto guard : candidate.discrete_asks) {
      os << "guard: " << get_infix_string(guard.first)
         << " on_time: " << guard.second << endl;
    }
    os << endl;
  }
  return os;
