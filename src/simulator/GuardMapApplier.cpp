...
void GuardMapApplier::visit_atomic_constraint(std::shared_ptr<symbolic_expression::Node> node)
{
...
void GuardMapApplier::visit(std::shared_ptr<symbolic_expression::LogicalAnd> node)
{
  accept(node->get_lhs());
...
void GuardMapApplier::visit(std::shared_ptr<symbolic_expression::LogicalOr> node)
{
  accept(node->get_lhs());
  constraint_t lhs_node = applied_node;
  accept(node->get_rhs());
