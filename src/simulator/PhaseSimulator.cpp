...
*/
    ...
    if(!aborting)
    {
      ...
      set<ask_t> calculated_pp_time_set;
      ...
      for(auto variable : diff_variables)
      {
        ...
        for(auto ask : asks)
        {
          ...
          if(!calculated_pp_time_set.count(ask))
          {
            variable_map_t related_vm = get_related_vm(ask->get_guard(), original_vm);
