...
phase_list_t PhaseSimulator::process_todo(phase_result_sptr_t &todo) {
  ...
  if (todo->parent == result_root.get()) {
    ...
    //     &diff_positives, &nonalways_asks);
    //   }
    // for (auto constraint :
    // non_always)relation_graph_->set_expanded_atomic(constraint, false); for
    // (auto ask : nonalways_asks)relation_graph_->set_expanded_atomic(ask,
    // false);
    // //
  } else {
    ...
    for (auto phase : phase_list) {
      ...
      for (auto var : *variable_set_) {
        ...
        if (var.get_differential_count() == 0 &&
            !phase->variable_map.count(var) && var.get_name().back() != '$') {
          warning_var_str += var.get_string() + " ";
          warning_var.insert(var);
        }
      }
      if (warning_var_str.length() > 0) {
        HYDLA_LOGGER_WARN(warning_var_str,
                          " is completely unbound at phase... \n", *phase);
        if (phase->current_time.get_string() == "0" and
            phase->end_time.get_string() == "0") {
          cout << "WARNING: " << warning_var_str << "is not initialized!"
               << endl;
        } else {
...
value_t PhaseSimulator::calculate_middle_value(const phase_result_sptr_t &phase,
                                               ValueRange range) {
  itvd itv = evaluate_interval(phase, range, false);
  double mid_double = (itv.lower() + itv.upper()) / 2;
  value_t mid_val(mid_double);
  backend_->call("transformToRational", false, 1, "vln", "vl", &mid_val,
                 &mid_val);
  return mid_val;
}
...
PhaseSimulator::make_results_from_todo(phase_result_sptr_t &todo) {
  ...
  // add assumptions
  // TODO: If the discrete_guard is not approximated, this process may be
  // redundant
  if (todo->phase_type == INTERVAL_PHASE) {
    for (auto it = todo->discrete_guards.begin();
         it != todo->discrete_guards.end();) {
  ...
  backend_->call("addParameterConstraint", true, 1, "csn", "",
                 &todo->additional_parameter_constraint);
  backend_->call("addAssumption", true, 1,
                 todo->phase_type == INTERVAL_PHASE ? "cst" : "csn", "",
                 &todo->additional_constraint_store);

  relation_graph_->set_ignore_prev(todo->phase_type == POINT_PHASE &&
                                   todo->parent != result_root.get());
  ...
  if (todo->phase_type == POINT_PHASE) {
    for (auto trigger : todo->discrete_asks) {
      if (trigger.second) {
        ...
        /// 離散変化の発生のトリガーとなるガード条件がprevのみを参照している場合,
        /// calculate_closureまで持っていく必要なく成立が確定する.
        if (relation_graph_->entail_if_prev(ask, !entailed)) {
          todo->always_list.add_constraint_store(
              relation_graph_->get_always_list(ask));
          ...
          if (entailed) {
            ...
            pair<std::map<Variable, Variable>, constraint_t> p =
                clone_exists_constraint(ask, todo->id);
            if (!p.first.empty()) {
  ...
  while (module_set_container->has_next()) {
    ...
    auto tmp_result_list =
        simulate_ms(unadopted_ms, todo, nonprev_trigger_asks);
  ...
  if (todo->profile["# of CheckConsistency"]) {
    todo->profile["Average of CheckConsistency"] =
        todo->profile["CheckConsistency"] /
        todo->profile["# of CheckConsistency"];
  }
  if (todo->profile["# of CheckEntailment"]) {
    todo->profile["Average of CheckEntailment"] =
        todo->profile["CheckEntailment"] /
        todo->profile["# of CheckEntailment"];
  }

  return result_list;
}

module_diff_t PhaseSimulator::get_module_diff(module_set_t unadopted_ms,
...
PhaseSimulator::simulate_ms(const module_set_t &unadopted_ms,
  ...
  bool consistent =
      calculate_closure(phase, trigger_asks, local_diff_sum, ms_local_positives,
                        ms_local_negatives, ms_local_always);
  ...
  for (auto constraint_store :
       consistency_checker->get_inconsistent_constraints()) {
  ...
  if (!consistent) {
    ...
    vector<variable_map_t> create_result =
        consistency_checker->get_result_maps();
    if (create_result.size() == 0) {
      throw HYDLA_ERROR(
          "some error occured in creation of variable maps at phase...\n" +
          phase->get_string());
    ...
    if (opts_->epsilon_mode > 0) {
      ...
      for (auto var_entry : before_map) {
        HYDLA_LOGGER_DEBUG("#epsilon before : ", var_entry.first, " : ",
                           var_entry.second);
      }
      ...
      for (auto var_entry : phase->variable_map) {
        HYDLA_LOGGER_DEBUG("#epsilon after : ", var_entry.first, " : ",
                           var_entry.second);
      }
    }
    ...
    phase->profile["# of CheckConsistency(Backend)"] +=
        consistency_checker->get_backend_check_consistency_count();
    phase->profile["CheckConsistency(Backend)"] +=
        consistency_checker->get_backend_check_consistency_time();
    ...
    if (phase->phase_type == POINT_PHASE && phase->in_following_step()) {
      ...
      std::map<std::string, int> tmp_vm =
          consistency_checker->get_differential_map(tmp_vs);
      for (auto pair : tmp_vm) {
        ...
        if (max_d_count < 0) {
          ...
          if (phase->variable_map.count(var) == 0 ||
              phase->prev_map.count(var) == 0 ||
              !phase->variable_map[var].unique() ||
              !phase->prev_map[var].unique() ||
              !check_equality(phase->variable_map[var].get_unique_value(),
                              phase->prev_map[var].get_unique_value()))
            discrete_vs.insert(var);
        }
      }
  ...
  revert_diff(ms_local_positives, ms_local_negatives, ms_local_always,
              module_diff);
  return result_list;
}
...
void PhaseSimulator::push_branch_states(phase_result_sptr_t original,
  ...
  branch_state_false->additional_parameter_constraint.add_constraint_store(
      replace_prev_store(original->parent, result.inconsistent_store));

  original->parent->todo_list.push_back(branch_state_false);
  original->additional_parameter_constraint.add_constraint_store(
      replace_prev_store(original->parent, result.consistent_store));
  ...
  backend_->call("addParameterConstraint", true, 1, "csn", "",
                 &original->additional_parameter_constraint);
  HYDLA_LOGGER_DEBUG_VAR(*branch_state_false);
  HYDLA_LOGGER_DEBUG_VAR(*original);
}

phase_result_sptr_t
PhaseSimulator::clone_branch_state(phase_result_sptr_t original) {
  ...
  branch_state_false->set_parameter_constraint(
      original->get_parameter_constraint());
  branch_state_false->additional_parameter_constraint =
      original->additional_parameter_constraint;

  branch_state_false->discrete_differential_set =
      original->discrete_differential_set;
...
ConstraintStore PhaseSimulator::replace_prev_store(PhaseResult *parent,
                                                   ConstraintStore orig) {
...
void PhaseSimulator::check_break_points(phase_result_sptr_t &phase,
  ...
  if (!break_point_list.empty()) {
    ...
    for (auto entry : break_point_list) {
      ...
      variable_map_t related_vm =
          get_related_vm(break_point.condition, variable_map);
      switch (consistency_checker->check_entailment(
          related_vm, cc_result, break_point.condition, phase->phase_type,
          phase->profile)) {
      case BRANCH_PAR:
...
void PhaseSimulator::initialize(variable_set_t &v, parameter_map_t &p,
                                variable_map_t &m,
  ...
  interval::AffineApproximator::get_instance()->set_extra_dummy_num(
      opts_->extra_dummy_num);
...
void PhaseSimulator::replace_prev2parameter(PhaseResult &phase,
                                            variable_map_t &vm) {
  PrevReplacer replacer(*phase.parent, *simulator_, backend_.get(),
                        opts_->affine);
  ...
  phase.set_parameter_constraint(
      replace_prev_store(phase.parent, phase.get_parameter_constraint()));
...
variable_set_t get_discrete_variables(ConstraintStore &diff_sum,
                                      PhaseType phase_type) {
  variable_set_t result;
  for (auto constraint : diff_sum) {
    VariableFinder finder;
    finder.visit_node(constraint);
    variable_set_t variables = phase_type == POINT_PHASE
                                   ? finder.get_variable_set()
...
bool PhaseSimulator::calculate_closure(phase_result_sptr_t &phase,
                                       ...
                                       ConstraintStore &expanded_always) {
  auto detail = logger::Detail(__FUNCTION__);
  ...
  do {
    ...
    if (phase->parent == result_root.get() && first) {
      ...
      first = false;
    } else if (phase->phase_type == POINT_PHASE ||
               !phase->in_following_step()) {
      for (auto variable : discrete_variables) {
        auto each_adjacents = relation_graph_->get_adjacent_asks(
            variable.get_name(), phase_type == POINT_PHASE);
      ...
      for (auto variable : discrete_variables) {
        if (phase->parent->discrete_differential_set.count(variable)) {
          auto each_adjacents =
              relation_graph_->get_adjacent_asks2var_and_derivatives(
                  variable, phase_type == POINT_PHASE);
          adjacents.insert(each_adjacents.begin(), each_adjacents.end());
        }
      }
    }
    ...
    for (auto ask_it = unknown_asks.begin();
      ...
      if (phase_type == POINT_PHASE
          // in initial phase, conditions about left-hand limits are considered
          // to be invalid
      ...
      case ENTAILED: // 成立する場合
        ...
        if (!relation_graph_->get_entailed(ask)) {
          ...
          if (negative_asks.erase(ask)) {
            ...
            expanded_always.add_constraint_store(
                relation_graph_->get_always_list(ask));
          ...
          pair<std::map<Variable, Variable>, constraint_t> p =
              clone_exists_constraint(ask, phase->id);
          if (!p.first.empty()) {
            map<Variable, Variable> vm = p.first;
            constraint_t conseq = p.second;
            for (auto entry : vm) {
            ...
            for(auto entry: phase->variable_map) {
              if(vm.count(entry.first.name) > 0){
                phase->variable_map[Variable(vm[entry.first.name],
            entry.first.get_differential_count())] = entry.second;
              }
            }
            */
          }
  ...
  while (true) {
    ...
    CheckConsistencyResult cc_result;
    cc_result = consistency_checker->check_consistency(
        *relation_graph_, diff_sum, phase_type, phase->profile, unknown_asks,
        phase->in_following_step());
...
variable_map_t PhaseSimulator::get_related_vm(const node_sptr &node,
...
find_min_time_result_t PhaseSimulator::find_min_time(
  ...
  if (opts_->step_by_step) {
    return find_min_time_step_by_step(guard, original_vm, time_limit, phase,
                                      entailed,
                                      atomic_guard_min_time_interval_map);
  }

  auto detail = logger::Detail(__FUNCTION__);

  HYDLA_LOGGER_DEBUG_VAR(get_infix_string(guard));
  std::list<AtomicConstraint *> guards =
      relation_graph_->get_atomic_guards(guard);
    ...
    if (!guard_time_map.count(g)) {
      ...
      if (opts_->interval && guard_by_newton.get() == nullptr) {
        if (opts_->guards_to_interval_newton.empty()) {
          cout << "apply Interval Newton method to " << get_infix_string(g)
               << "?('y' or 'n')" << endl;
          char c;
          cin >> c;
          if (c == 'y') {
      ...
      if (!by_newton) {
        ...
        variable_map_t related_vm = get_related_vm(g, original_vm);
        backend_->call("substituteVM", true, 3, "etmvtvlt", "e", &cons,
                       &related_vm, &phase->current_time, &cons);

        value_t lower("0");
        backend_->call("calculateConsistentTime", true, 2, "etvlt", "e", &cons,
                       &lower, &constraint_for_this_guard);
  ...
  if (guard_by_newton.get() != nullptr) {
    ...
    backend_->call("substituteVM", true, 3, "etmvtvlt", "e", &guard_by_newton,
                   &related_vm, &phase->current_time, &time_guard_by_newton);
    ...
    parameter_map_t pm = parameter_map_vector.size() == 1
                             ? parameter_map_vector.front()
                             : parameter_map_t();
    list<itvd> result_interval_list =
        calculate_interval_newton_nd(time_guard_by_newton, pm);
    ...
    if (guard_type == typeid(Equal) || guard_type == typeid(UnEqual)) {
      ...
      while (true) {
        ...
        backend_->call("resetConstraintForParameter", false, 1, "mp", "",
                       &pm_for_each);
        ...
        if (empty) {
          ...
          guard_time_map[guard_by_newton] = constraint_t(new UnEqual(
              new se::Parameter(parameter_current), new SymbolicT()));
        } else {
          guard_time_map[guard_by_newton] = constraint_t(
              new Equal(new se::Parameter(parameter_current), new SymbolicT()));
        }
        constraint_t lb, ub;
        lb.reset(
            new LessEqual(new se::Parameter(parameter_prev), new SymbolicT()));
        ub.reset(new LessEqual(new SymbolicT(),
                               new se::Parameter(parameter_current)));
        constraint_t time_bound;
        // prev_t < t /\ t < current_t
        time_bound.reset(new LogicalAnd(lb, ub));

        min_time_for_this_ask = min_time_calculator.calculate_min_time(
            &guard_time_map, guard, entailed, time_limit, time_bound);
      ...
      itvd prev_interval = itvd(0, 0);
      bool include_border = (guard_type == typeid(se::LessEqual) ||
                             guard_type == typeid(se::GreaterEqual));
      ...
      while (true) {
...

        {
          constraint_t lb, ub;
          if (include_border) {
            lb.reset(new LessEqual(new se::Parameter(parameter_lower),
                                   new SymbolicT()));
            ub.reset(new LessEqual(new SymbolicT(),
                                   new se::Parameter(parameter_upper)));
          } else {
            lb.reset(
                new Less(new se::Parameter(parameter_lower), new SymbolicT()));
            ub.reset(
                new Less(new SymbolicT(), new se::Parameter(parameter_upper)));
          }
          if (!negated)
            guard_time_map[guard_by_newton] =
                constraint_t(new LogicalAnd(lb, ub));
          else
            guard_time_map[guard_by_newton] =
                constraint_t(new Not(new LogicalAnd(lb, ub)));
        }
        ...
        constraint_t time_bound;
        {
          ...
          if (negated)
            ...
            lb.reset(new LessEqual(new se::Parameter(parameter_prev),
                                   new SymbolicT()));
          ub.reset(
              new Less(new SymbolicT(), new se::Parameter(parameter_upper)));
        ...
        // TODO: deal with  branching of cases
        if (!min_time_for_this_ask.empty() &&
            !min_time_for_this_ask.front().time.infinite()) {
          HYDLA_ASSERT(min_time_for_this_ask.size() == 1);
          ...
          new_store.add_constraint_store(lower_range.create_range_constraint(
              node_sptr(new se::Parameter(parameter_lower))));
          new_store.add_constraint_store(upper_range.create_range_constraint(
              node_sptr(new se::Parameter(parameter_upper))));
          phase->set_parameter_constraint(new_store);
          backend_->call("resetConstraintForParameter", false, 1, "csn", "",
                         &new_store);

          min_time_for_this_ask.front().range_by_newton =
              ValueRange(value_t(parameter_lower), value_t(parameter_upper));
          break;
        }
        ...
        if (!result_interval_list.empty()) {
          ...
          // deal with remaining interval ( ub of upper_interval ->
          // upper_bound_of_itv_newton)
          negated = true;
    ...
    min_time_for_this_ask = min_time_calculator.calculate_min_time(
        &guard_time_map, guard, entailed, time_limit);
  }
...
PhaseSimulator::StateOfIntervalNewton
PhaseSimulator::initialize_newton_state(const constraint_t &time_guard,
                                        parameter_map_t &pm) {
  StateOfIntervalNewton state;
  backend_->call("relationToFunction", true, 1, "et", "e", &time_guard,
                 &state.exp);
  backend_->call("differentiateWithTime", true, 1, "et", "e", &state.exp,
                 &state.dexp);
...
find_min_time_result_t PhaseSimulator::find_min_time_step_by_step(
  ...
  for (auto atomic_guard : guards) {
    ...
    backend_->call("substituteVM", true, 3, "etmvtvlt", "e", &g, &related_vm,
                   &phase->current_time, &time_guard);
    if (typeid(*(time_guard)) == typeid(True)) {
      // TODO: consider to improve efficiency by exploiting the information that
      // this guard will be forever true
      ...
      if (by_newton) {
        ...
        backend_->call("solveTimeEquation", true, 2, "etvlt", "f", &time_guard,
                       &lower, &time_list_for_this_guard);
      ...
      if (guard_type == typeid(Equal) || guard_type == typeid(UnEqual)) {
        atomic_guard_map[atomic_guard->constraint] =
            (guard_type == typeid(Equal));
      } else {
        bool true_at_initial_time;
        backend_->call("trueAtInitialTime", true, 2, "etvlt", "b", &time_guard,
                       &lower, &true_at_initial_time);
        atomic_guard_map[atomic_guard->constraint] = true_at_initial_time;
      }
    }
  }

  ConstraintStore current_parameter_constraint =
  ...
  for (auto it = newton_guard_state_map.begin();
    ...
    if (atomic_guard_min_time_interval_map.find(atomic_guard_str) !=
        atomic_guard_min_time_interval_map.end()) {
  ...
  while (true) {
    ...
    for (auto it = newton_guard_state_map.begin();
         it != newton_guard_state_map.end(); ++it) {
      const int atomic_guard_index =
          std::distance(newton_guard_state_map.begin(), it);
      auto &entry = *it;
      ...
      if (!state.min_interval) {
        const bool generate_new_data =
            current_atomic_guard_index_history_stack_index.find(
                atomic_guard_index) ==
        ...
        if (!generate_new_data) {
          ...
          if (stack_states.size() == stack_index) {
            ...
            new_data.current_stack_top =
                std::make_shared<kv::interval<double>>(state.stack.top());

            state.min_interval = interval::calculate_interval_newton(
                state.stack, state.exp, state.dexp, pm,
                !phase->in_following_step() ||
                    phase->discrete_guards.count(entry.first));

            new_data.min_interval = std::make_shared<kv::interval<double>>(
                state.min_interval.value());
      ...
      if (new_data_opt) {
        ...
        if (opts_->affine) {
          ...
          backend_->call("calculateTLinear", true, 4, "etmpvltvlt", "ct",
                         &state.exp, &pm, &lb, &ub, &ct);
          ValueRange range;
          range.set_upper_bound(value_t("1"), true);
          range.set_lower_bound(value_t("-1"), true);
          value_t new_value = ct.exp;
          new_value += value_t(ct.mid_rad.midpoint +
                               ct.mid_rad.radius * value_t(parameter));
        ...
        atomic_guard_min_time_interval_map[atomic_guard_str].results.push_back(
            new_data);
        if (current_atomic_guard_index_history_stack_index.find(
                atomic_guard_index) ==
            current_atomic_guard_index_history_stack_index.end()) {
        ...
        elem = TimeListElement(stack_states[stack_index].time_list_element_time,
                               entry.first);
    ...
    backend_->call("resetConstraintForParameter", false, 1, "mp", "",
                   &pm_for_each);
    backend_->call("getMinimum", true, 1, "tl", "f", &time_list,
                   &minimum_candidates);
    ...
    for (auto &candidate : minimum_candidates) {
      ...
      for (int index : candidate.guard_indices) {
        ...
        if (newton_it != newton_guard_state_map.end()) {
          ...
          const int atomic_guard_index =
              std::distance(newton_guard_state_map.begin(), newton_it);
          ++current_atomic_guard_index_history_stack_index[atomic_guard_index];
        } else
          symbolic_guard_times_map[guard].pop_front();
      }
      bool on_time;
      bool satisfied = checkAndUpdateGuards(atomic_guard_map, guard, guard_list,
                                            on_time, entailed);
      if (satisfied) {
        ...
        if (guard_it != guard_parameter_map.end()) {
          ...
          if (simulator_parameter_map.find(new_param) ==
              simulator_parameter_map.end()) {
            simulator_->introduce_parameter(new_param, pm_for_each[new_param]);
          }
        }
        backend_->call("productWithNegatedConstraint", true, 2, "csncsn", "cs",
                       &current_parameter_constraint,
                       &candidate.parameter_constraint,
                       &current_parameter_constraint);
...
bool PhaseSimulator::checkAndUpdateGuards(map<constraint_t, bool> &guard_map,
                                          constraint_t guard,
                                          list<constraint_t> guard_list,
                                          bool &on_time, bool entailed) {
  ...
  for (auto &atomic_guard : guard_list) {
    ...
    if (guard_type == typeid(Equal) || guard_type == typeid(LessEqual) ||
        guard_type == typeid(GreaterEqual)) {
  ...
  backend_->call("isGuardSatisfied", true, 1, "en", "b", &applied_guard,
                 &satisfied);
...
void PhaseSimulator::remove_redundant_parameters(phase_result_sptr_t phase) {
  ...
  backend_->call("removeRedundantParameters", true, 4, "vltvltmvncsn", "cs",
                 &phase->current_time, &end_time, &phase->variable_map,
                 &par_cons, &par_cons);
...
void PhaseSimulator::make_next_todo(phase_result_sptr_t &phase) {
  ...
  if (phase->phase_type == POINT_PHASE) {
    ...
    phase->variable_map =
        value_modifier->shift_time(phase->current_time, phase->variable_map);
    ...
    if (phase->in_following_step()) {
      ...
      for (auto var_entry : vm_to_take_over) {
        ...
        if (!phase->variable_map.count(var) &&
            relation_graph_->referring(var)) {
          phase->variable_map[var] = vm_to_take_over[var];
          // create a variable map whose start point(t=0) is the start of this
          // phase
          timer::Timer shift_time_timer;
          original_vm[var] = value_modifier->shift_time(-phase->current_time,
                                                        vm_to_take_over[var]);
          phase->profile["ShiftTime"] += shift_time_timer.get_elapsed_us();
        }
      }
    ...
    if (!aborting) {
      if (phase->in_following_step()) {
        ...
        for (auto &entry : phase->next_pp_candidate_map) {
          for (auto cand_it = entry.second.begin();
               cand_it != entry.second.end();) {
            auto &candidate = *cand_it;

            candidate.time -=
                (phase->current_time - phase->parent->parent->current_time);
            backend_->call("productWithGlobalParameterConstraint", true, 1,
                           "csn", "cs", &candidate.parameter_constraint,
                           &candidate.parameter_constraint);
            if (!candidate.parameter_constraint.consistent())
              ...
              for (auto &entry : candidate.other_guards_to_time_condition) {
                ...
                entry.second =
                    value_modifier
                        ->shift_time(-(phase->current_time -
                                       phase->parent->parent->current_time),
                                     value_t(entry.second))
                        .get_node();
                phase->profile["ShiftTime"] +=
                    shift_time_timer.get_elapsed_us();
              }
              cand_it++;
            }
          }
      ...
      MinTimeCalculator min_time_calculator(relation_graph_.get(),
                                            backend_.get());
      set<ask_t> asks;
      ...
      if (opts_->interval && !max_time.infinite()) {
        upper_bound_of_itv_newton =
            evaluate_interval(phase, max_time - phase->current_time, false)
                .upper();
      }

      for (auto ask : asks) {
        candidate_map[ask] = find_min_time(
            ask->get_guard(), min_time_calculator, guard_time_map, original_vm,
            time_limit, relation_graph_->get_entailed(ask), phase,
            atomic_guard_min_time_interval_map);
      }
      ...
      for (auto &entry : break_point_list) {
        auto break_point = entry.first;
        entry.second =
            find_min_time(break_point.condition, min_time_calculator,
                          guard_time_map, original_vm, time_limit, false, phase,
                          atomic_guard_min_time_interval_map);
      }
      ...
      phase->profile["CompareMinTime"] +=
          compare_min_time_timer.get_elapsed_us();
      /*
            if (opts_->epsilon_mode >= 0){
         ...
         entry.parameter_constraint);
              ...
              time_result = reduce_unsuitable_case(time_result, backend_.get(),
         phase);
      ...
      */
      ...
      if (time_result.empty()) {
        ...
        while (true) {
          ...
          backend_->call("simplify", false, 1, "vln", "vl", &phase->end_time,
                         &phase->end_time);
          if (candidate.time.undefined() || candidate.time.infinite()) {
            ...
            if (opts_->interval) {
              ...
              if (evaluate_interval(phase,
                                    value_t(upper_bound_of_itv_newton) -
                                        candidate.time,
                                    false)
                      .lower() < 0) {
                string asks_str = "";
                ...
                throw HYDLA_ERROR("failed to calculate valid time of the "
                                  "discrete change for " +
                                  asks_str);
              }
            }
            if (opts_->step_by_step) {
            ...
            } else if (opts_->interval || opts_->numerize_mode ||
                       opts_->epsilon_mode > 0) {
              ...
              for (auto discrete_ask : next_todo->discrete_asks) {
                ...
                if (candidate.guard_by_newton.get() != nullptr) {
                  backend_->call(
                      "borderIsIncluded", true, 3, "vlnvlnvln", "b",
                      &candidate.time,
                      &candidate.range_by_newton.get_lower_bound().value,
                      &candidate.range_by_newton.get_upper_bound().value,
                      &included_by_newton_guard);
                ...
                for (auto guard : candidate.other_guards_to_time_condition) {
                  ...
                  backend_->call("onBorder", true, 2, "etvlt", "b",
                                 &guard.second, &candidate.time, &on_border);
                  if (on_border) {
                    // if (included_by_newton_guard)throw HYDLA_ERROR("Both of 2
                    // guards are on the border: " +
                    // get_infix_string(guard.first) +", " +
                    // get_infix_string(candidate.guard_by_newton));
            ...
            next_todo->set_parameter_constraint(
                phase->get_parameter_constraint());
            next_todo->parent = phase.get();
            timer::Timer apply_time_timer;
            next_todo->prev_map =
                value_modifier->substitute_time(candidate.time, original_vm);
            phase->profile["ApplyTime2Expr"] +=
                apply_time_timer.get_elapsed_us();
...
void PhaseSimulator::approximate_phase(phase_result_sptr_t &phase,
  ...
  if (opts_->numerize_mode) {
    phase->current_time =
        calculate_middle_value(phase, ValueRange(phase->current_time));
    for (auto &entry : vm_to_approximate) {
      entry.second.set_unique_value(
          calculate_middle_value(phase, entry.second));
    }
  } else if (opts_->interval && opts_->approximation_step > 0 &&
             (phase->step / 2) % opts_->approximation_step == 0) {
    if (opts_->affine) {
      interval::AffineApproximator *approximator =
          interval::AffineApproximator::get_instance();
      vector<parameter_map_t> parameter_maps = phase->get_parameter_maps();
      assert(parameter_maps.size() == 1);
      approximator->approximate(vars_to_approximate, vm_to_approximate,
                                parameter_maps[0], phase->current_time);
      backend_->call("resetConstraintForParameter", false, 1, "mp", "",
                     &parameter_maps[0]);
      phase->set_parameter_constraint(get_current_parameter_constraint());
    } else {
...
pp_time_result_t
PhaseSimulator::compare_min_time(const pp_time_result_t &existings,
                                 const find_min_time_result_t &newcomers,
                                 const ask_t &ask) {
  ...
  if (existings.empty()) {
    for (auto newcomer : newcomers) {
      ...
      DCCandidate candidate(newcomer.time, discrete_asks,
                            newcomer.parameter_constraint);
    ...
    for (auto newcomer : newcomers) {
      for (auto existing : existings) {
        ...
        backend_->call("compareMinTime", true, 4, "vltvltcsncsn", "cp",
                       &existing.time, &newcomer.time,
                       &existing.parameter_constraint,
                       &newcomer.parameter_constraint, &compare_result);

        if (compare_result.less_cons.consistent()) {
          DCCandidate candidate(existing.time, existing.discrete_asks,
                                compare_result.less_cons);
          result.push_back(candidate);
        }
        if (compare_result.equal_cons.consistent()) {
          ...
          if (ask.get())
            discrete_asks[ask] = newcomer.on_time;
          DCCandidate candidate(existing.time, discrete_asks,
                                compare_result.equal_cons);
          result.push_back(candidate);
        }
...
void PhaseSimulator::revert_diff(const PhaseResult &phase) {
  revert_diff(phase.get_diff_positive_asks(), phase.get_diff_negative_asks(),
              phase.always_list, phase.module_diff);
}

void PhaseSimulator::revert_diff(const asks_t &positive_asks,
...
list<itvd>
PhaseSimulator::calculate_interval_newton_nd(const constraint_t &time_guard,
                                             parameter_map_t &pm) {
  node_sptr exp;
  node_sptr dexp;
...
itvd PhaseSimulator::evaluate_interval(const phase_result_sptr_t phase,
  ...
  if (!use_affine) {
    ...
    return interval_visitor.get_interval_value(
        range.get_unique_value().get_node(), nullptr, &pm);
  } else {
    interval::AffineTreeVisitor affine_visitor(phase->variable_map);
    return affine_visitor.approximate(range.get_unique_value().get_node())
        .to_interval();
  }
}

void PhaseSimulator::add_parameter_constraint(const phase_result_sptr_t phase,
                                              const Parameter &parameter,
                                              ValueRange range) {
  simulator_->introduce_parameter(parameter, range);
  ConstraintStore new_store = phase->get_parameter_constraint();
  new_store.add_constraint_store(
      range.create_range_constraint(node_sptr(new se::Parameter(parameter))));
  phase->set_parameter_constraint(new_store);
 ...
 * @brief この関数はexecution stuck(unsatisfiable
 * constraints)時に原因となったモジュールを求めるときに呼ばれるため，requiredでないモジュールは除去する
 * @param (causes) 引数の説明
 * @return フィルター後のマップ
...
void PhaseSimulator::print_possible_causes(
    const map<variable_set_t, module_set_t> &mp) {
...
void PhaseSimulator::update_condition(const variable_set_t &vs,
