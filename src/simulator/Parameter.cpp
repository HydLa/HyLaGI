...
Parameter::Parameter(const Variable &variable, const PhaseResult &phase)
    : variable_name_(variable.get_name()),
      differential_count_(variable.get_differential_count()),
      phase_id_(phase.id) {}
...
Value Parameter::as_value() const {
  return Value(
      symbolic_expression::node_sptr(new symbolic_expression::Parameter(
          get_name(), get_differential_count(), get_phase_id())));
}
...
Parameter::Parameter(const std::string &str) {
  ...
  if (prev_index == string::npos || index == string::npos ||
      prev_index >= index)
    throw runtime_error("invalid parameter name " + str);
  ...
  if (prev_index == string::npos || index == string::npos ||
    ...
    throw runtime_error("invalid parameter name " + str);
  differential_count_ =
      atoi(str.substr(prev_index + 1, index - (prev_index + 1)).c_str());
  prev_index = index;
  index = str.find_first_of("]", index + 1);
  if (prev_index == string::npos || index == string::npos ||
      prev_index >= index)
...
bool ParameterComparator::operator()(const Parameter x,
                                     const Parameter y) const {
  return x < y;
}
