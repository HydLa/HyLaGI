...
ConsistencyChecker::ConsistencyChecker(backend_sptr_t back)
    : backend(back), prev_map(nullptr), backend_check_consistency_count(0),
      backend_check_consistency_time(0) {}
ConsistencyChecker::~ConsistencyChecker() {}

void ConsistencyChecker::send_range_constraint(const Variable &var,
  ...
  if (range.unique()) {
    ...
    if (range.get_lower_cnt()) {
      ...
      if (range.get_lower_bound().include_bound) {
        std::string name =
            prev_mode ? "addPrevGreaterEqual" : "addGreaterEqual";
        backend->call(name.c_str(), false, 2, fmt.c_str(), "", &var, &value);
      } else {
...
ConsistencyChecker::call_backend_check_consistency(const PhaseType &phase,
  ...
  if (phase == POINT_PHASE) {
    backend->call("checkConsistencyPoint", true, 1, "csn", "cc", &tmp_cons,
                  &ret);
    for (auto consistent : ret.consistent_store)
      HYDLA_LOGGER_DEBUG_VAR(get_infix_string(consistent));
    for (auto inconsistent : ret.inconsistent_store)
      HYDLA_LOGGER_DEBUG_VAR(get_infix_string(inconsistent));
  } else {
...
CheckEntailmentResult ConsistencyChecker::check_entailment(
    RelationGraph &relation_graph, CheckConsistencyResult &cc_result,
    const constraint_t &guard,
    const constraint_t &constraint_for_default_continuity,
    const asks_t &unknown_asks, const PhaseType &phase, bool following_step,
    profile_t &profile) {
  ...
  backend->call("addConstraint", true, 1,
                (phase == POINT_PHASE) ? "csn" : "cst", "", &constraint_store);
  return check_entailment_essential(cc_result, guard, phase, profile);
}

CheckEntailmentResult ConsistencyChecker::check_entailment(
    variable_map_t &vm, CheckConsistencyResult &cc_result,
    const constraint_t &guard, const PhaseType &phase, profile_t &profile) {
  HYDLA_LOGGER_DEBUG_VAR(get_infix_string(guard));
...
void ConsistencyChecker::check_consistency_foreach(
    const ConstraintStore &constraints, RelationGraph &relation_graph,
    CheckConsistencyResult &result, const PhaseType &phase, profile_t &profile,
    bool following_step) {
  VariableFinder finder;
  ...
  if (!tmp_result.consistent_store.consistent()) {
    ...
    inconsistent_module_sets.push_back(
        relation_graph.get_related_modules(constraints));
    ...
    if (tmp_result.inconsistent_store.consistent()) {
      ...
      result.inconsistent_store.add_constraint_store(
          tmp_result.inconsistent_store);
      return;
    }
    result.consistent_store.add_constraint_store(tmp_result.consistent_store);

    if (result_maps.size() == 0)
      return; // if result_maps has already been invalidated, skip creation
    ...
    backend->call("getSizeOfConstraint", false, 0, "", "i",
                  &size_of_constraint);
    ...
    if (create_result.size() == 1) {
      ...
      //     HYDLA_LOGGER_DEBUG("#epsilon create result varialbe map ",i," :
      ...
      result_maps.clear(); // invalidate result_maps (caused by over-constrained
                           // or under-constrained)
    }
  }
}

CheckConsistencyResult ConsistencyChecker::check_consistency(
    RelationGraph &relation_graph, ConstraintStore &difference_constraints,
    const PhaseType &phase, profile_t &profile, const asks_t &unknown_asks,
    bool following_step) {
  ...
  relation_graph.get_related_constraints_vector(
      difference_constraints, related_constraints_list, module_set_vector);
  ...
  for (int i = 0; i < related_constraints_list.size(); i++) {
    ...
    check_consistency_foreach(related_constraints_list[i], relation_graph,
                              result, phase, profile, following_step);
    if (!result.consistent_store.consistent())
      break;
    if (result.consistent_store.consistent() &&
        !result.inconsistent_store.empty())
      break;
  }
...
CheckConsistencyResult ConsistencyChecker::check_consistency_essential(
    const ConstraintStore &constraint_store, VariableFinder &finder,
    const PhaseType &phase, profile_t &profile, bool following_step) {
  timer::Timer timer;
