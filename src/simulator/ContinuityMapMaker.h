...
public:
  ...
  void visit_node(std::shared_ptr<symbolic_expression::Node> node,
                  const bool &in_IP, const bool &negative) {
    in_interval_ = in_IP;
    negative_ = negative;
   ...
   * 初期状態に戻す
   */
  void reset() { variables_.clear(); }

  void set_continuity_map(const continuity_map_t &map) { variables_ = map; }

  continuity_map_t get_continuity_map() { return variables_; }
...
std::ostream &operator<<(std::ostream &s,
                         const continuity_map_t &continuity_map);
