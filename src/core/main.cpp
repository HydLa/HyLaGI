...
void add_vars_from_string(string var_string, set<string> &set_to_add,
                          string warning_prefix);
bool process_opts(Opts &opts, ProgramOptions &p, bool use_default);
void add_vars_from_string(string vars_list_string, set<string> &set_to_add,
                          string warning_prefix);
...
int hydla_main(int argc, char *argv[]) {
  ...
  if (option_pos != string::npos) {
    ...
    option_string = comment.substr(
        option_pos, pos == string::npos ? string::npos : pos - option_pos);
  }
  ...
  //マクロ処理
  {
    ...
    public:
      MacroDefinition(const std::string &name, const std::string &argments,
                      const std::string &definition)
          : m_name(name), m_argments(splitArgments(argments)),
      ...
      void apply(std::string &input, std::string::size_type offset) const {
        if (debugPrint) {
          ...
          std::cout << "apply target {"
                    << std::string(input.cbegin() + offset, input.cend())
                    << "}\n";
        }
        ...
        if (m_argments.empty()) {
          for (string::size_type pos = input.find(m_name, offset);
               pos != string::npos; pos = input.find(m_name, pos)) {
            {
              ...
              if (pos + m_name.length() < input.length() &&
                  isAlNum(input[pos + m_name.length()])) {
                pos += m_name.length();
                continue;
              }
            }
          ...
          for (string::size_type pos = input.find(m_name, offset);
            ...
            const auto argments = splitArgments(std::string(
                input.cbegin() + start, input.cbegin() + currentPos));
    ...
    private:
      ...
      splitArgments(const std::string &argments) {
        ...
        const std::string rawStr(argments.cbegin() + left + 1,
                                 argments.cbegin() + right);
        if (debugPrint) {
...

        {
          ...
          while (true) {
            ...
            if (rawStr[next] == ',' && nestDepth == 0) {
              result.emplace_back(rawStr.cbegin() + startPos,
                                  rawStr.cbegin() + next);
              if (debugPrint) {
                std::cout << "    add \"" << result.back() << "\"\n";
              }
      ...
      std::string
      appliedDefinition(const std::vector<std::string> &argments) const {
        ...
        for (size_t i = 0; i < m_argments.size(); ++i) {
          for (string::size_type pos = result.find(m_argments[i]);
               pos != string::npos; pos = result.find(m_argments[i], pos)) {
            {
              ...
              if (pos + m_argments[i].length() < result.length() &&
                  isAlNum(result[pos + m_argments[i].length()])) {
                pos += m_argments[i].length();
                continue;
              }
            }
    ...
    for (string::size_type pos = comment.find(define_header);
         pos != string::npos; pos = comment.find(define_header, pos)) {
      if (debugPrint) {
        std::cout << "=======================================\n";
      }

      std::smatch match;
      if (std::regex_search((comment.cbegin() + pos), comment.cend(), match,
                            reg)) {
  ...
  if (isOmit || isOutput) {
    ...
    // 省略対象指定部を取り出す
    string::size_type pos = comment.find('\n', output_pos);
    string var_string = comment.substr(
        output_pos, pos == string::npos ? string::npos : pos - output_pos);

    add_vars_from_string(var_string, opts.output_vars,
                         string("[") + (isOmit ? "#omit" : "#output") + "]");
  }
  ...
  if (cmdline_options.count("parse_only") ||
      options_in_source.count("parse_only")) {
    HYDLA_LOGGER_STANDARD("successfully parsed");
    return 0;
  }

  if (cmdline_options.count("debug_constraint") ||
      options_in_source.count("debug_constraint")) {
  ...
  HYDLA_LOGGER_STANDARD("Simulation Time : ",
                        simulation_timer.get_time_string());
