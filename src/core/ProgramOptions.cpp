...
void ProgramOptions::init_descriptions()
{
  options_description generic_desc("Usage: hylagi [options] [file]\n\nAllowed options",
                                   LINE_LENGTH);
  generic_desc.add_options()("help,h", "display help message (this option)")("version", "display version")

      ("parse_only", "only try to parse given program")("dump_parse_tree", "only output parse tree")("dump_parse_tree_json", "only output parse tree in JSON format")("debug_constraint", "debugging program")

          ("dump_module_set_graph",
           "only output candidate sets of module sets\n"
           "  in graph representation")("dump_module_priority_graph",
                                        "only output priorities of modules\n"
                                        "  in graphviz format")("dump_relation_graph",
  ...
  toggle_desc.add_options()("nd", value<char>()->default_value('n'), "nondeterministic mode")
                          ...
                          ("static_generation_of_module_sets", value<char>()->default_value('n'), "simulate with static generation of module sets")
                              ...
                              ("ignore_warnings", value<char>()->default_value('n'),
                               ...
                               "Note: Warnings from HyLaGI itself are always activated")
                                  ...
                                  ("interval,i", value<char>()->default_value('n'), "use interval method")("numerize_without_validation", value<char>()->default_value('n'), "numerize values of variables at the end of each PointPhase")("eager_approximation", value<char>()->default_value('n'), "approximate values in advance of each Point Phase")("approximation_step", value<int>()->default_value(1), "the interval of step to approximate value of variable")("extra_dummy_num", value<int>()->default_value(0), "the number of extra dummy parameters in affine arithmetic.")

                                      ("partition_num", value<int>()->default_value(1), "the number of partitions of parameters at PP1 in nonlinear mode.")

                                          ("dump_in_progress", value<char>()->default_value('n'),
                                           "output each phase in progress")("use_shorthand", value<char>()->default_value('n'), "use shorthands for arithmetic expressions (only for parameters)\n")("vars_to_approximate",
                                                                                                                                                                                                     value<std::string>()->default_value(""),
                                                                                                                                                                                                     "variables to approximate (delimited by \",\")")("guards_to_interval_newton",
                                                                                                                                                                                                                                                      value<std::string>()->default_value(""),
                                                                                                                                                                                                                                                      "guards to be solved by interval newton method(delimited by \",\")")("step_by_step", value<char>()->default_value('n'),
                                                                                                                                                                                                                                                                                                                           "use find_min_time_step_by_step")("solve_over_reals", value<char>()->default_value('n'), "solve constrants over the reals")("html", value<char>()->default_value('n'), "output log with HTML format");
...
void ProgramOptions::parse(int argc, char* argv[])
{
  ...
  store(command_line_parser(argc, argv).options(cmdline_desc_).positional(positional_opt).extra_parser(reg_toggle).run(), vm_);
  notify(vm_);
}
