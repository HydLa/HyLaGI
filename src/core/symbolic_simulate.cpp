...
void output_result(Simulator &ss, Opts &opts) {
  ...
  // todo : この std::cout << std::endl; を外すと、ログの出力が遅延して detail
  // タグが先に閉じてしまう...
  std::cout << std::endl;
  ...
  while (std::getline(sstr, buffer, ',')) {
    ...
    if (!regex_search(buffer, match, re)) {
      cout << warning_prefix << " warning : \"" << buffer
           << "\" is not a valid variable name." << endl;
    }
...
int simulate(std::shared_ptr<hydla::parse_tree::ParseTree> parse_tree) {
  ...
  backend_.reset(new Backend(
      new MathematicaLink(opts.wstp, opts.ignore_warnings, opts.simplify_time,
                          opts.simplify, opts.dsolve, opts.solve_over_reals)));
