...
void output_result(Simulator& ss, Opts& opts)
{
  ...
  {
    Printer.set_epsilon_mode(backend_, true);
  }
...
void process_opts(Opts& opts, ProgramOptions& po, bool use_default)
{
  ...
  opts.static_generation_of_module_sets = po.count("static_generation_of_module_sets") && po.get<char>("static_generation_of_module_sets") == 'y';
  IF_SPECIFIED("dump_in_progress")
  opts.dump_in_progress = po.count("dump_in_progress") > 0 && po.get<char>("dump_in_progress") == 'y';
  opts.dump_relation = po.count("dump_relation_graph") > 0;
  IF_SPECIFIED("ignore_warnings")
  opts.ignore_warnings = po.count("ignore_warnings") > 0 && po.get<char>("ignore_warnings") == 'y';
  IF_SPECIFIED("use_shorthand")
  TreeInfixPrinter::set_use_shorthand(po.count("use_shorthand") > 0 && po.get<char>("use_shorthand") == 'y');
  IF_SPECIFIED("ha")
  ...
  opts.numerize_mode = po.count("numerize_without_validation") > 0 && po.get<char>("numerize_without_validation") == 'y';
  IF_SPECIFIED("eager_approximation")
  opts.eager_approximation = po.count("eager_approximation") > 0 && po.get<char>("eager_approximation") == 'y';
  IF_SPECIFIED("fail_on_stop")
  opts.stop_at_failure = po.count("fail_on_stop") > 0 && po.get<char>("fail_on_stop") == 'y';
  ...
  IF_SPECIFIED("partition_num")
  opts.partition_num = po.get<int>("partition_num");
  ...
  opts.step_by_step = po.count("step_by_step") > 0 && po.get<char>("step_by_step") == 'y';
  IF_SPECIFIED("simplify")
  opts.simplify = po.get<int>("simplify");
  IF_SPECIFIED("solve_over_reals")
  opts.solve_over_reals = po.count("solve_over_reals") > 0 && po.get<char>("solve_over_reals") == 'y';
  IF_SPECIFIED("html")
