...
AffineApproximator *AffineApproximator::get_instance() {
  if (affine_translator == NULL)
    affine_translator = new AffineApproximator();
...
void AffineApproximator::set_simulator(Simulator *s) { simulator = s; }

void AffineApproximator::set_extra_dummy_num(int num) { extra_dummy_num = num; }

void AffineApproximator::reduce_dummy_variables(
    kv::ub::vector<affine_t> &formulas, int limit) {
  std::map<int, int> index_map = kv::epsilon_reduce(formulas, limit);
  for (auto pair : index_map) {
    parameter_idx_map_t::right_iterator r_it =
        parameter_idx_map.right.find(pair.first);
...
value_t AffineApproximator::translate_into_symbolic_value(
    const affine_t &affine_value, parameter_map_t &parameter_map) {
  // set rounding mode
  kv::hwround::roundup();
  value_t ret(affine_value.a(0));
  simulator->backend->call("transformToRational", true, 1, "vln", "vl", &ret,
                           &ret);
  double sum = 0;
  int available_index;
  for (int i = 1; i < affine_value.a.size(); i++) {
    ...
    if (r_it == parameter_idx_map.right.end()) {
      ...
      // TODO:
      // kvライブラリ内のダミー変数も削除する？結果に誤りは生まれないはずだが、インデックスが無駄に増える。
      sum += affine_value.a(i);
      available_index = i;
    } else {
      // convert floating point into rational
      value_t val = value_t(affine_value.a(i));
      simulator->backend->call("transformToRational", true, 1, "vln", "vl",
                               &val, &val);
  ...
  if (sum != 0) {
    ...
    // parameters whose differential counts are -1 are regarded as dummy
    // variables
    parameter_t param =
        simulator->introduce_parameter("affine", -1, ++epsilon_index, range);
    ...
    simulator->backend->call("transformToRational", true, 1, "vln", "vl", &val,
                             &val);
...
void AffineApproximator::approximate(
    const simulator::variable_set_t &vars_to_approximate,
    variable_map_t &variable_map, parameter_map_t &parameter_map,
    value_t &time) {
  ...
  // 試験的にダミー変数の削減をしてみる
  // TODO:外部から削減タイミングを指定するようにする
  kv::ub::vector<affine_t> formulas(affine_map.size() +
                                    (time_is_affine ? 1 : 0));
  int i = 0;
  if (time_is_affine) {
    formulas[0] = time_val.type == INTERVAL ? affine_t(time_val.interval)
                                            : time_val.affine_value;
    i = 1;
  }
