...
bool empty(itvd x) { return x.lower() > x.upper(); }
...
bool show_existence(itvd candidate, node_sptr exp, node_sptr dexp,
  ...
  while (true) {
    ...
    if (max_width <= min_width || w == max_width || w == min_width)
      throw HYDLA_ERROR(
          "show existence failed for exp:" + get_infix_string(exp) +
          " dexp: " + get_infix_string(dexp) + ". ");
    ...
    itvd f_result =
        visitor.get_interval_value(exp, &time_interval, &phase_map_);
...
double calculate_mid_without_0(itvd current_interval, node_sptr exp,
                               parameter_map_t &phase_map_) {
  using namespace std;
  itvd f_result;
  ...
  while (!rate_queue.empty()) {
    ...
    double m =
        current_interval.lower() * (1 - rate) + current_interval.upper() * rate;
    itvd m_itv = itvd(m);
    ...
    if (!in(0., f_result)) {
      ...
      for (int i = 0; i < STEP; i++) {
        double child_rate_sup =
            rate_inf + (rate_sup - rate_inf) * (i + 1) / STEP;
...
itvd calculate_interval_newton(itv_stack_t &candidate_stack, node_sptr exp,
                               node_sptr dexp, parameter_map_t &phase_map_,
                               bool discard_itv_0) {
  bool parted;
  itvd current_interval, prev_interval, div1, div2, nx, nx2, x2, f_result,
      d_result, m;
  ...
  while (!candidate_stack.empty()) {
    ...
    while (true) {
      ...
      IntervalTreeVisitor visitor;
      itvd whole_range =
          visitor.get_interval_value(exp, &current_interval, &phase_map_);
      ...
      d_result =
          visitor.get_interval_value(dexp, &current_interval, &phase_map_);
      if (in(0., d_result)) {
        double mid_val =
            calculate_mid_without_0(current_interval, exp, phase_map_);
        if (mid_val == INVALID_MID) {
          throw HYDLA_ERROR("No valid intermediate values are found in "
      ...
      if (i >= MAX_STEP) {
        HYDLA_LOGGER_WARN("Maximum step(=" + std::to_string(MAX_STEP) +
                          ") is exceeded. The result width may be large.");
        break;
      }
    }
    if (!no_zero) {
      if (!discard_itv_0 && in(0., current_interval))
        throw HYDLA_ERROR("invalid interval found for " +
                          get_infix_string(exp));
      else if (!in(0., current_interval) &&
               show_existence(current_interval, exp, dexp, phase_map_)) {
...
std::list<itvd> calculate_interval_newton_nd(itvd init, node_sptr exp,
                                             node_sptr dexp,
                                             parameter_map_t &phase_map_) {
  bool parted;
  itvd current_interval, prev_interval, div1, div2, nx, nx2, x2, f_result,
  ...
  while (!candidate_stack.empty()) {
    ...
    if (!empty(current_interval) && !in(0., current_interval) &&
        show_existence(current_interval, exp, dexp, phase_map_)) {
      HYDLA_LOGGER_DEBUG("FIND");
      HYDLA_LOGGER_DEBUG_VAR(width(current_interval));
      HYDLA_LOGGER_DEBUG_VAR(current_interval);
