...
public:
   ...
   */
  void approximate(const simulator::variable_set_t &vars_to_approximate,
...
private:
   ...
   * @param limit the number which the number of dummy variables are to be after
   * reduction
   */
  void reduce_dummy_variables(kv::ub::vector<affine_t> &formulas, int limit);

  value_t translate_into_symbolic_value(const affine_t &affine_value,
