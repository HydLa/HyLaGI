...
namespace simulator
{
NonlinearReplacer::NonlinearReplacer(const variable_map_t& map, bool v_to_par) : variable_map(map), v_to_par(v_to_par)
{
}

NonlinearReplacer::~NonlinearReplacer()
{
}

void NonlinearReplacer::replace_node(symbolic_expression::node_sptr& node)
{
  differential_cnt = 0;
  replace_cnt = 0;
  new_child_.reset();
...
void NonlinearReplacer::replace_value(value_t& val)
{
...
void NonlinearReplacer::replace_range(ValueRange& range)
{
  if (range.unique())
  {
...
void NonlinearReplacer::visit(boost::shared_ptr<hydla::symbolic_expression::Variable> node)
{
...
void NonlinearReplacer::visit(boost::shared_ptr<hydla::symbolic_expression::Differential> node)
{
  differential_cnt++;
...
void NonlinearReplacer::visit(boost::shared_ptr<hydla::symbolic_expression::Parameter> parameter)
{
  if (v_to_par)
  {
...
#define DEFINE_DEFAULT_VISIT_ARBITRARY(NODE_NAME)                  \
  void NonlinearReplacer::visit(boost::shared_ptr<NODE_NAME> node) \
  {                                                                \
