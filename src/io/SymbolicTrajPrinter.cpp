...
SymbolicTrajPrinter::SymbolicTrajPrinter(backend_sptr_t b, std::ostream &ost,
                                         bool numerize_par)
    : ostream(ost), backend(b), numerize_parameter(numerize_par) {}

string
SymbolicTrajPrinter::get_state_output(const phase_result_t &result) const {
  stringstream sstr;
  ...
  if (epsilon_mode_flag && backend.get()) {
    output_limits_of_variable_map(sstr, backend.get(), result,
                                  result.variable_map);
  }
  return sstr.str();
}

void SymbolicTrajPrinter::output_asks(std::ostream &stream,
...
void SymbolicTrajPrinter::output_inconsistent_constraints(
    std::ostream &stream, const phase_result_t &phase) const {
...
void SymbolicTrajPrinter::output_parameter_map(
    const parameter_map_t &pm) const {
...
void SymbolicTrajPrinter::output_variable_map(
    std::ostream &stream, const phase_result_t &result) const {
  ...
  for (auto it = vm.begin(); it != vm.end(); ++it) {
    ...
    if (opts.interval && it->second.unique() &&
        result.phase_type == POINT_PHASE) {
...
void SymbolicTrajPrinter::output_one_phase(
    const phase_result_const_sptr_t &phase, const std::string &prefix) const {
  ostream << prefix << endl;
  ...
  if (par_maps.size() > 0) {
    if (par_maps.size() == 1) {
      ...
      for (auto it = par_maps.begin(); it != par_maps.end(); it++, i++) {
        ostream << "---------parameter condition(" << i + 1 << ")---------"
                << endl;
...
void SymbolicTrajPrinter::output_result_tree(
    const phase_result_const_sptr_t &root) const {
  if (root->children.size() == 0) {
    ostream << "No Result." << endl;
    return;
  }
  int i = 1, j = 1;
  phase_result_sptrs_t::const_iterator it = root->children.begin(),
...
void SymbolicTrajPrinter::output_result_node(
    const phase_result_const_sptr_t &node, vector<std::string> &result,
    int &case_num, int &phase_num) const {
  if (node->simulation_state == simulator::NOT_SIMULATED)
    return;
  if (node->children.size() == 0) {
...

    {
      ...
      if (par_maps.size() > 0) {
        if (par_maps.size() == 1) {
          if (!par_maps[0].empty()) {
            ostream << "---------parameter condition(Case" << current_case_num
                    << ")---------" << endl;
            output_parameter_map(par_maps[0]);
          }
        } else {
    ...
    phase_result_sptrs_t::const_iterator it = node->children.begin(),
...
void SymbolicTrajPrinter::output_limit_of_time(
    std::ostream &stream, Backend *backend,
  ...
  if (result.phase_type == INTERVAL_PHASE) {
    if (!result.end_time.undefined() && !result.current_time.undefined()) {
      ...
      backend->call("checkEpsilon", true, 1, "en", "i", &tmp_current_time,
                    &check_current_time);
      backend->call("checkEpsilon", true, 1, "en", "i", &tmp_end_time,
                    &check_end_time);
      check_result = check_current_time * check_end_time;
      if (check_result == 1) {
        backend->call("limitEpsilon", true, 1, "en", "vl", &tmp_current_time,
                      &ret_current_time);
        backend->call("limitEpsilon", true, 1, "en", "vl", &tmp_end_time,
                      &ret_end_time);
        stream << "Limit(t)\t: " << ret_current_time << "->" << ret_end_time
               << "\n";
      } else {
        backend->call("limitEpsilonP", true, 1, "en", "vl", &tmp_current_time,
                      &ret_current_time);
        backend->call("limitEpsilonP", true, 1, "en", "vl", &tmp_end_time,
                      &ret_end_time);
        stream << "Limit(t)\t+ : " << ret_current_time << "->" << ret_end_time
               << "\n";
        backend->call("limitEpsilonM", true, 1, "en", "vl", &tmp_current_time,
                      &ret_current_time);
        backend->call("limitEpsilonM", true, 1, "en", "vl", &tmp_end_time,
                      &ret_end_time);
        stream << "Limit(t)\t- : " << ret_current_time << "->" << ret_end_time
               << "\n";
      }
...
void SymbolicTrajPrinter::output_limits_of_variable_map(
    std::ostream &stream, Backend *backend, const phase_result_t &result,
...
void SymbolicTrajPrinter::output_property_automaton(PropertyNode *node) {
  ...
  // "\"" << ";" << "\n"; dump_property_automaton(node); ostream << "}" << "\n";
  // node->write_reset();
}
void SymbolicTrajPrinter::dump_property_automaton(PropertyNode *node) {
  ...
  //     ostream << ";" << "\n";
  //   }
  //   for (Property_link_t::iterator it = node->link.begin();it !=
  //   node->link.end();it++)
  //   {
...
void SymbolicTrajPrinter::dump_ltl_node(simulator::LTLNode *node) {
  ...
  //     }
  //   }
  //   for (ltl_node_list_t::iterator it = node->link.begin();it !=
  //   node->link.end();it++)
  //   {
  //     if (node->id == (*it)->id)
  //     {
