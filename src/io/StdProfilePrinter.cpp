...
void StdProfilePrinter::print_profile(const entire_profile_t &result) const {
  ...
  cout << endl << endl;
  for (auto phase : result) {
    for (int i = 0; i < (max_characters + max_digits - 3) / 2; i++)
      cout << "-";
    cout << " ";
    ...
    for (auto entry : phase->profile) {
      cout << entry.first;
      for (int i = entry.first.length(); i < max_characters; i++)
        cout << " ";
      cout << " : ";
      for (int i = 0; i < max_digits - std::to_string(entry.second).length();
           i++)
        cout << " ";
